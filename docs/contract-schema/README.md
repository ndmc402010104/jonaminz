# Contract JSON Schema — RC3.1（定案，Worker 端已實作並部署上線）
...
**現況（2026-07-11）**：這份 schema 已經是 Worker 端 `submitContract` action
實際在用的驗證依據（`backend/cloudflare-worker/contract-schema-validator.
generated.js` 是從這份 schema build-time 預編譯出來的，見 backend/README.md）。
欄位形狀仍算**未鎖死**——因為還沒有任何真實外部專案的合約被 approve 過
（`integration-settings.json` 目前是空的），S11「已發布欄位永不改語意與型別」
要等第一個真實合約 approve 後才正式對這份 schema 生效；但改動已經有實際 Worker
程式碼在依賴，改 schema 之後記得重跑 `generate-contract-validator.mjs` 並重新
`wrangler deploy`，不再只是改個草稿檔案而已。
...
真正的安全邊界一定要用標準 URL parser 在 Worker 端重算一次**——完整檢查清單記在
`platform-integration-v1-implementation-plan.md` 第 2 項，`backend/cloudflare-
worker/contract-validation.js` 的 `validateUrls()` 已依此實作並部署。
...
- 上面第 3、4 點已定案；第 5 點的 `$id` 待正式發布時處理（見 checklist），
  這是唯一還沒收尾的項目，不擋任何後續工作。
- **implementation plan 第 2 項（Worker 端合約收取）已完成並部署上線**：
  immutable snapshot 三態、audit table、S15 全部防線（URL parser／origin／
  credential 檢查、pre-parse request size 限制）、environment-scoped
  registered origin 資料模型、`requests`/`requires` ⊆ `supports` 的 cross-field
  檢查、`entryId`/`objectType` 參照與重複檢查全部已實作，細節見
  `backend/README.md`。所有寫入一律先進 pending，不得因提交 Contract 自動
  approve 或 grant（S13, S16）——目前也還沒有任何寫入曾被 approve 過。
- 下一步是 implementation plan 第 3 項（核准後台）。真正的「已登記專案 →
  正確 environment/Origin → 合法 Contract → pending snapshot → audit log」
  正向成功路徑，留到第一個真實外部專案登記時一併驗證（2026-07-11 使用者
  裁決，不用現在特地建假專案測）。
