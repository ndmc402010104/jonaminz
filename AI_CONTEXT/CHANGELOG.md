...
- **變更**（`assets/js/chat-thread.js`＋兩份 CSS）：
  - **檔案/圖片下載進 ⋮ 操作列**（使用者待辦板補的「下載是不是加載
    更多裡面的功能就好了」）：render 時在圖片/檔案訊息的 message 元素
    上帶 `data-download-item-id/owner/name`，`openActionSheet` 多一顆
    「⬇️下載」；下載邏輯抽成 `triggerChatFileDownload()` helper，檔案
    泡泡點擊跟 ⋮ 選單共用（都導覽到 /downloadChatFile）。圖片訊息
    原本只能點開放大、沒地方下載，現在 ⋮ 也能下載。
  - **貼圖/常用回覆面板**（task 9b2157f2，決策圖候選畢業）：composer
    加一顆 💬 切換鍵（在 🙂 左邊），開一個面板含「貼圖」（16 個大
    emoji，點了直接送 emoji-only 訊息、會自動放大顯示）＋「常用回覆」
    （10 句預設短句，點了直接送）。兩人自用先寫死一組 `STICKER_SET`／
    `CANNED_REPLIES`（之後要可編輯再說，不現在過度設計）。textarea
    右邊距加大容納兩顆按鈕。
  - `document` click 收面板改成同時關 emoji 跟 quick 兩個面板。
- **驗證**：harness（in-page dispatch）通過——檔案訊息 ⋮ 有「⬇️下載」
  且點了觸發下載；💬 開面板 16 貼圖／10 短句，點貼圖送「❤️」、點
  短句送「收到 👌」，送完面板收起。
- **版本**：v0.46.40-202607162328
...
- **任務**：使用者兩個需求——(1)「手機長按又跳表情又跳功能列」改成
  跟電腦版 hover 一樣的 ⋮/↩/🙂 工具列，收斂到一個地方；(2) 表情反應
  要 optimistic（「按下去等好久」）。
- **變更**（`assets/js/chat-thread.js`＋兩份 CSS）：
  - **手機長按 → ⋮/↩/🙂 工具列**：新增 `openTouchToolbar()`，長按浮出
    一排 ⋮/↩/🙂（用 `els.contextMenu` position:fixed 浮層承載，貼觸摸
    點上方）——不再同時彈「表情列＋底部操作列」。⋮ 開底部操作列、
    ↩ 直接回覆、🙂 原地換成表情選單。`openContextMenu` 改成只叫
    `openTouchToolbar`。新增 `.jonaminz-chat-touch-toolbar` CSS（跟
    電腦版 hover 工具列同款膠囊外觀）。使用者選「純三顆比照電腦版」。
  - **表情反應 optimistic**：第一版直接改本機資料，但被 1.5秒 poll 用
    「後端還沒處理完」的舊資料蓋掉→使用者回報「換太快會彈回去」。
    改成覆蓋層 `reactionOverride`（messageId→想要的 emoji）：render
    把覆蓋套在後端資料上（後端是一人一個反應，套法＝拿掉我在後端的
    反應換成覆蓋值），poll 進來也照套，直到後端資料追上覆蓋值才在
    render 裡自動清掉（比對相符就 delete）。toggle 呼叫成功後主動
    poll 一次加速確認。
- **驗證**：harness（in-page dispatch）三個情境都過：(a) 長按出
  ⋮/↩/🙂、🙂→表情選單、點表情立刻出現 pill；(b) 換表情樂觀替換
  （👍→❤️ 立刻只剩❤️、再點清空）；(c) **防彈回**——模擬真實伺服器
  （toggle 400ms 延遲＋poll 競爭），快速 👍→❤️ 後 3 秒取樣 12 次全部
  穩定「❤️ 1」，沒有彈回👍或消失。
- **遺留**：使用者截圖回報「還是會出現這個」（畫面有兩張圖片顯示成
  灰色載入條）——尚未釐清「這個」確切指哪個，可能是舊圖片的 Graph
  縮圖還在載入/生成，待使用者確認後再查。待辦板使用者新增兩筆：
  檔案下載想放進「⋮更多」選單、貼圖/常用回覆面板想現在做。排程
  系統設計已定案（純看板高亮不推播／一次性到期／獨立排程區）尚未
  動工。
- **版本**：v0.46.35-202607162236
...
- **任務**：使用者確認刪除終於成功後回報兩件事：(1)「剛進入畫面沒有
  CACHE 的時候圖片載入還是很慢」；(2) 縮小視窗＋重整浮動大頭貼會
  消失、手機轉螢幕大頭貼卡在畫面正中間。
- **變更**：
  - **圖片冷載入**（`worker.js` `getImageUrls`＋`chat-thread.js`＋
    localStorage 快取結構）：慢的主因是位元組——「選檔案」路徑上傳
    的照片是原檔直傳（動輒數 MB），冷載入要整張抓完。改成泡泡顯示
    用 **Graph 自動生成的縮圖**（`$expand=thumbnails` 的
    `large`≈800px、幾十 KB），`getImageUrls` 回傳多一個 `thumbs`
    map；點開放大（lightbox 讀新的 `data-full-url`）／下載仍用全尺寸
    downloadUrl。縮圖 URL 一樣進 localStorage 持久化（store 條目多
    `tu` 欄位，向下相容舊格式）。縮圖拿不到就 fallback 全尺寸，功能
    不退化。註：`$expand` 不觸發 `$select` 那個「downloadUrl 靜默
    消失」的怪癖（那是精簡查詢限定），已一併寫在程式碼註解。
  - **浮動大頭貼邊界**（`chat-launcher.js`）：儲存的自由位置是舊視窗
    尺寸下的絕對座標，原本只在載入時夾 top、left 原樣還原——視窗變
    窄後 left 落在畫面外（消失）、轉螢幕後座標對不上新方向（卡在
    怪位置）。改成：(a) 載入還原時 left 也照「吸最近的邊」規則重算；
    (b) 新增 `reclampToViewport()`，`resize`（150ms debounce）／
    `orientationchange`（延遲 300ms 等新尺寸生效）時把目前位置重新
    夾回合法範圍＋重吸最近邊；沒拖過（還在預設錨點）不處理——錨點
    本來就相對右上角，自己會跟著視窗。
- **驗證**：`node --check` 三檔通過。縮圖需 Worker deploy 後生效；
  真機/瀏覽器行為待使用者實測（縮圖顯示、點開放大是全尺寸、縮放
  視窗/轉螢幕泡泡跟著回到合法位置）。
- **版本**：v0.46.31-202607162114
...
- **任務**：第十二筆上線後使用者回報四項仍失敗（刪除、反白、捲動鎖、
  分享卡片沒有刪除選項）。這輪先查資料庫拿到決定性事實：
  `chat_messages` 全表**從來沒有任何一筆 `deleted_at` 非空**——刪除
  功能從 7/14 上線起就沒成功過，不是這兩天的回歸。再用 harness
  實測前端整條路徑（hover→⋮→刪除→確認→呼叫後端）完全正常，唯一
  無法在 harness 重現的環節是 `window.confirm()`：瀏覽器（Edge/
  Chrome）在頁面連續彈出對話框後會提供「不要再允許顯示對話方塊」，
  勾過一次之後 **confirm 永遠直接回傳 false**，刪除就在確認這關
  無聲中止——完全吻合「按了沒反應、沒錯誤、資料庫沒請求」。第十二
  筆補的 alert 錯誤提示同樣會被壓掉，等於沒補。
- **變更**（`assets/js/chat-thread.js`）：
  - **刪除確認完全不用瀏覽器對話框**：改成兩段式按鈕——第一下按鈕
    變「⚠️確定刪除？」（3 秒內沒按第二下自動復原），第二下顯示
    「⏳刪除中…」並真的呼叫後端；失敗直接把原因寫在按鈕上
    （「❌失敗：<原因>」）＋狀態列＋console.error，不再用 alert。
  - **分享卡片（shared_item）也可刪除**：`canDeleteMedia` 從列舉
    image/file 改成「自己傳的任何未刪除訊息都可刪」（文字訊息走
    原本的 editable 路徑，不重複出按鈕）。
  - **反白選取第二個根因**：反白拖曳放開滑鼠那一刻的 click 會觸發
    「點訊息看時間」→ render() 重建 DOM → 選取被清掉。有未收合的
    選取範圍（`getSelection().isCollapsed === false`）就跳過時間
    切換。
  - **捲動鎖第二個根因**：鎖訊息串 overflow 擋不住落在遮罩上的
    滾輪/觸控捲動「鏈」到外層宿主頁面（面板是 iframe），在遮罩上
    直接 `preventDefault` wheel/touchmove（passive:false）。
- **驗證**：harness（Playwright＋stub backend）實測兩段式刪除全流程
  ——文字訊息操作列四顆按鈕齊全、檔案訊息單獨一顆刪除、第一下變
  確認態、第二下真的發出 deleteChatMessage、成功後收合，全部通過。
  反白/捲動鎖是邏輯修正，視覺行為待使用者實測。過程中 harness 一度
  失控（多份背景測試互咬、殭屍瀏覽器、Playwright locator.click 的
  hit-target 檢查對這個 fixed 定位操作列會誤判逾時——改用
  dispatchEvent 驗證邏輯），拖了很久才收斂。
- **遺留**：`window.confirm` 假說無法百分之百證實（沒辦法遠端看到
  使用者瀏覽器的對話框設定），但即使假說錯誤，新版兩段式＋按鈕內
  錯誤顯示也保證「失敗一定看得到原因」，下一輪回報就能終結。
- **版本**：v0.46.29-202607162044

---

## 2026-07-16（晚上，第十二筆）— Chat 一批修正：圖片可刪除（含 OneDrive 檔案本體）、URL 快取持久化、載入佔位框、反白選取、閃爍、捲動鎖、反應膠囊疊角

- **任務**：使用者連續回報七件事，一批處理：(1) 圖片沒辦法按更多功能；
  (2) 圖片要可以刪除（「傳錯的照片一直刪不掉」）；(3) 一進聊天室圖片
  是「細細的一條」沒載入；(4) hover 工具列每隔固定時間閃一下；(5) 心情
  膠囊不該算在訊息高度裡（工具列對齊跑掉）；(6) 對話文字沒辦法反白
  選取；(7) 底部操作列開著時背景還能捲動。另外使用者回報「刪除是假的
  （連文字都是）」——已知前端把後端 `ok:false` 完全忽略（靜默失敗），
  這輪先補上錯誤顯示，下次失敗會把真正原因顯示在狀態列。
- **變更**：
  - **圖片/檔案可刪除**（`chat-thread.js`＋`worker.js`）：前端拿掉
    「圖片/檔案不開放刪除」的舊限制（`canDeleteMedia`＋
    `data-deletable`），⋮／長按操作列對自己的圖片/檔案長出「刪除」；
    ⋮ 按鈕改成「至少有一個可用動作」才渲染（對方的圖片沒有動作就
    不出現，免得按了沒反應像壞掉）。Worker `deleteChatMessage` 軟刪除
    成功後，順手把 OneDrive 上的檔案本體一起刪（跟 purge 同一套
    Graph DELETE、404 視為成功；best-effort，失敗不影響訊息刪除，
    殘檔最終由 180 天 purge 撿走）並標記 `metadata.expired`。
  - **刪除失敗要說出來**（`chat-thread.js`）：`deleteChatMessage` 回
    `ok:false` 原本被 `.then(poll)` 完全忽略——畫面上什麼都不發生，
    看起來就是「假的刪除」。改成顯示「刪除失敗：<原因>」。
  - **URL 快取持久化**（`chat-thread.js`）：Graph downloadUrl 記憶體
    快取加一層 localStorage（50 分鐘 TTL，URL 本身約 1 小時效期），
    重進頁面直接命中、圖片幾乎秒開；載入失敗的 error-retry 路徑同步
    清掉持久化條目。
  - **載入佔位框**（兩份 CSS）：`img.is-loading` 給滿版寬度＋底色，
    配合 aspect-ratio 撐出正確比例的佔位框，不再是細細一條。
  - **反閃爍**（`chat-thread.js`）：render() 組出來的 HTML 跟上一次
    完全相同就不動 DOM——1.5 秒 poll 一次的全量 innerHTML 重建會把
    滑鼠下的 hover 工具列砍掉重建（opacity 從 0 重新淡入），就是
    使用者看到的規律閃爍。
  - **反應膠囊疊角**（兩份 CSS）：表情反應膠囊改成絕對定位疊在泡泡
    右下角外緣（Messenger 做法），不再自己佔一行高度——訊息列高度
    等於泡泡本身，hover 工具列的 flex-end 對齊才正確；有反應的訊息
    用 `:has()` 補下邊距避免壓到下一則。
  - **反白選取**（`chat-thread.js`）：長按選單＋左滑回覆手勢改成只
    對觸控/手寫筆生效（`pointerType === "mouse"` 直接 return）——
    滑鼠拖曳反白原本被滑動手勢劫持（往左拖 preventDefault）、按住
    不動 480ms 誤觸長按選單；桌機有 hover 工具列，這兩套手勢本來
    就不該吃滑鼠事件。
  - **捲動鎖**（`chat-thread.js`）：底部操作列開啟時鎖住訊息串
    `overflow`，關閉恢復。
- **遺留**：
  1. 「刪除是假的」的真正根因還沒確認——錯誤顯示上線後，使用者下次
     刪除失敗狀態列會顯示具體原因（LOGIN_REQUIRED／NOT_FOUND_OR_
     FORBIDDEN…），拿到那個字串才能對症。
  2. Playwright harness 這輪失控（多份背景測試互咬＋殭屍瀏覽器），
     這批修改**只過了語法檢查、沒有完成視覺驗證**——與既定紀律不符，
     是在使用者即時回報壓力下的取捨，下一輪要先把 harness 跑穩。
  3. Worker 變更（刪訊息連檔案）需要 deploy 才生效；未 deploy 前
     刪圖片只會軟刪除訊息、檔案本體留著。
- **版本**：v0.46.28-202607162019

---

## 2026-07-16（晚上，第十一筆）— hover 工具列最終定案：Messenger 式同行並排＋sticky 垂直定位，拆掉全部 JS 定位邏輯

- **任務**：使用者拿真的 Messenger 截圖定案——工具列（⋮／↩／🙂）要
  跟泡泡**並排在同一行**（自己的訊息在泡泡左邊、對方的在右邊），不是
  浮在上方/下方。之前五輪的「絕對定位＋智慧翻轉」全部是方向性錯誤：
  我一直在「上面還是下面」打轉，使用者從第一次就說「那一行都是它的，
  hover 過去就那一行**旁邊**選」——「旁邊」兩個字講得很清楚，是我
  一直沒聽懂。
- **變更**：
  - `assets/js/chat-thread.js`：整段 `mouseover`／`is-below` 定位判斷
    **全部刪除**——工具列改成訊息列裡的普通 flex item（DOM 順序：
    大頭貼→泡泡容器→工具列），`data-mine="true"` 的 row-reverse 自然
    把它排到泡泡左邊、對方訊息排右邊，零 JS。
  - 兩份 CSS：`position:absolute`／`is-below` 整段換成 flex item＋
    `align-self:flex-end`（貼訊息最下緣）＋ `position:sticky;
    bottom:8px`——使用者追加的需求「單行就在那一行；多行貼這則訊息
    最下緣；訊息長到超出畫面就貼可視範圍的最下緣，才不會長訊息
    點不到」，sticky 一條屬性全部涵蓋，不需要 JS 量測。外觀同時
    比照 Messenger 拿掉膠囊背景框，只留圖示。
  - 順帶修正測試流程：這輪開始所有修改都先在 Playwright harness
    （stub JonaminzBackend＋真 chat-thread.js＋真 CSS）截圖驗證過
    才交付——超長訊息（2654px 高 vs 627px 可視）工具列正確扣在
    585px 可視下緣、單行訊息正確貼在該行旁邊，兩張截圖都確認過。
- **狀態變化**：hover 工具列定位從「連續五輪 patch」收斂為結構性
  正確的最終版；使用者已確認方向（「終於對了」）＋追加的 sticky
  需求已實作並自驗，待使用者最後確認。
- **遺留**：`_test-panel-host.html` 測試檔已從 repo 刪除。本機另有
  兩個測試用靜態伺服器行程（port 8934/8935）還在跑，重開機自然消失。
- **版本**：v0.46.27-202607161937

---

## 2026-07-16（傍晚，第十筆五度追加）— 用 Playwright 實測抓到真根因：工具列蓋住時間分隔線，不是跟訊息脫節

- **任務**：前四次修正都是憑使用者的文字/截圖描述猜著改，使用者忍無可忍
  （這裡如實記錄使用者原話，方便未來一棒理解為什麼要改流程：「幹你媽的
  就是堅持不改就對了你到底甚麼毛病阿」）。這次改變方法：**不再用猜的**
  ——自己搭一個 Playwright 測試環境，塞進使用者截圖裡同樣的資料
  （一則很長的文字訊息接一則有時間分隔線的圖片訊息），實際截圖驗證，
  才找到真正的根因。
- **除錯過程**：用 Playwright 載入真正的 `chat-thread.js`＋兩份 CSS
  （stub `window.JonaminzBackend` 提供假訊息資料），模擬滑鼠 hover
  在圖片訊息上並截圖比對。結果完全重現使用者回報的畫面：⋮🙂
  工具列浮在「12:38」時間標籤上方一段距離，看起來完全跟下面的圖片
  脫節。量測 DOM 座標後找到真根因：`chat-thread.js` 在訊息密度不夠時
  會插入 `.jonaminz-chat-time-divider`（時間分隔線），這個分隔線跟
  下一則訊息之間只隔 `.jonaminz-chat-thread` 的 `gap`（幾 px）——
  遠小於工具列需要的高度（~32px）。上一筆的 `is-below` 判斷邏輯只看
  「離視窗頂端多遠」，量不到「離上一個兄弟元素多近」這種情況，工具列
  照樣往上貼、直接蓋住/貼在時間分隔線那裡，才會看起來「飄在半空中」。
- **變更**（`assets/js/chat-thread.js`）：`mouseover` 判斷邏輯改成
  量測「上一個兄弟元素（`previousElementSibling`，可能是時間分隔線、
  未讀分隔線，也可能是另一則訊息）的下緣」到「這則訊息上緣」的實際
  距離，不夠放工具列才改貼下方；沒有上一個兄弟元素、或它已經捲出
  視窗上方，退回視窗頂端（0）當基準，涵蓋原本「畫面最上面那則訊息」
  的情境，兩種判斷合併成同一套邏輯。
- **驗證**：Playwright 截圖直接證實——三則測試訊息（長文字／兩則各自
  接在時間分隔線後面的圖片）hover 後工具列都正確貼齊自己那則訊息的
  邊緣，不再蓋到分隔線或脫節懸空，跟真的 Messenger 截圖比對行為一致。
  測試用的 `_test-harness.html` 跟本機靜態伺服器都只在暫存環境跑，
  沒有進 repo。
- **給下一棒的教訓**：這條表情反應功能前後修了六輪都靠使用者文字/
  截圖回饋猜著改，直到自己動手用 Playwright 重現畫面才抓到真根因
  ——之後遇到「已經改了但視覺還是不對」這種回饋循環超過兩輪，就該
  直接搭測試環境親眼驗證，不要繼續憑描述猜。
- **版本**：v0.46.26-202607161819

---

## 2026-07-16（傍晚，第十筆四度追加）— hover 判定範圍改成整列撐滿寬度，不再只跟著窄泡泡縮

- **任務**：使用者附真的 Messenger 截圖比對回報「人家是判定整個橫向的
  大框框不是只有文字框格裡面」——`.jonaminz-chat-message` 原本
  `max-width:82%` 是加在「這一列自己」身上，這一列的實際寬度會跟著
  內容（大頭貼＋泡泡）縮到多窄，泡泡本身很窄（例如短訊息）時，可以
  觸發 hover 的範圍也跟著窄，滑鼠稍微往上移動離開這個窄範圍，
  `:hover` 就斷了——這才是「超難點」真正的根因，不是前幾筆修的間距
  問題（那些也是必要的，但沒解決範圍太窄本身）。
- **變更**（兩份 CSS）：`.jonaminz-chat-message` 拿掉 `max-width:82%`
  ／`align-self`，改成 `width:100%`（撐滿整個聊天串寬度）＋
  `justify-content:flex-start`，配合既有的 `data-mine="true"` 
  `flex-direction:row-reverse` 把大頭貼／泡泡推到該在的那一側——視覺
  結果完全不變，但整條可以觸發 hover 的範圍變成整列寬（跟真的
  Messenger 一樣），不再只有貼著文字的窄框。原本的 82%／86% 寬度上限
  改搬到 `.jonaminz-chat-bubble-col` 身上，只限制泡泡本身的視覺寬度，
  不影響 hover 判定範圍。
- **驗證**：CSS 花括號配對數雙檔皆通過。
- **版本**：v0.46.25-202607161806

---

## 2026-07-16（傍晚，第十筆三度追加）— hover 工具列改成智慧翻轉：上方沒空間就貼下方

- **任務**：使用者附截圖回報——畫面捲動後最上面那則訊息，hover 工具列
  固定往上貼，直接跑到可視範圍外面（被瀏覽器視窗頂端切掉），完全點
  不到。使用者原話「我不要頂端啦，你很堅持耶，頂端請問我這個怎麼
  點啊」。
- **變更**（`assets/js/chat-thread.js`／兩份 CSS）：純 CSS 沒辦法
  判斷「這則訊息上方還有沒有空間」，改用 JS 補這一段判斷：
  `els.thread` 新增 `mouseover` 監聽，量測被 hover 的
  `.jonaminz-chat-message` 的 `getBoundingClientRect().top`，小於
  工具列高度＋12px 緩衝就幫工具列加上 `is-below` class，CSS 對應改成
  `top:100%`（貼在訊息下方）取代預設的 `bottom:100%`（貼上方）；空間
  夠的話維持貼上方不變。上下兩種貼法都延續上一筆的「0 間距直接貼齊」
  原則，不會重新引入 hover 中途斷掉的問題。
- **驗證**：`node --check`／CSS 花括號配對數雙檔皆通過。
- **版本**：v0.46.24-202607161802

---

## 2026-07-16（傍晚，第十筆再追加）— 修正 hover 工具列中途消失、超難點到

- **任務**：使用者回報「這樣改超難點，你就做成那一行文字一整行都切成
  是它的，hover 過去就那一行旁邊選就好了」——上一筆用 `bottom:100%`
  ＋`margin-bottom:4px` 解決蓋字問題後，滑鼠從泡泡往上移到工具列
  中間會經過那 4px 的空白間距，那段距離不屬於任何元素，`:hover`
  狀態在移動途中就斷掉，工具列在滑鼠還沒到之前就先消失了。
- **變更**：`margin-bottom: 4px` 改成 `margin-bottom: 0`（直接貼齊
  整列頂端，不留間距）——滑鼠從泡泡內部往上移動，全程都在
  `.jonaminz-chat-message` 或工具列其中一個的可 hover 範圍內，不會
  斷。
- **驗證**：CSS 花括號配對數雙檔通過。
- **版本**：v0.46.23-202607161759

---

## 2026-07-16（傍晚，第十筆追加）— 修正 hover 工具列蓋到短訊息文字

- **任務**：使用者附截圖回報 hover 工具列蓋到訊息文字（短訊息「成功」
  被蓋住一部分）。
- **變更**：`.jonaminz-chat-hover-toolbar` 原本用 `top: -14px`（整頁版）
  ／`-12px`（攜帶版）猜一個固定像素當間距，短泡泡（泡泡本身矮）時
  這個間距不夠、工具列會蓋到文字。改成 `bottom: 100%`（貼齊整列
  `.jonaminz-chat-message` 的頂端，不是猜像素值）＋`margin-bottom: 4px`
  留間距——不管泡泡多高，工具列一定完整浮在上面，不會重疊。
- **驗證**：CSS 花括號配對數雙檔通過。
- **版本**：v0.46.22-202607161755
...
- **任務**：上一筆上線後使用者立刻回報「符號擋到文字格子了」——右下角
  常駐反應按鈕沒有留空間，短訊息最後一行文字會被小圓按鈕蓋到。
- **變更**：`.jonaminz-chat-bubble`（文字泡泡）加 `padding-right`（整頁版
  `calc(var(--space-4) + 20px)`，攜帶版固定 `26px`，配合各自原本的
  基礎字級/間距），保留出按鈕的空間，跟 file-bubble／shared-card
  上一筆已經做過的處理一致。
- **驗證**：CSS 花括號配對數雙檔通過。
- **版本**：v0.46.19-202607161737
...
- **任務**：使用者針對「電腦版下載體驗」的回饋附了兩張真實 Messenger
  截圖說明，文字補充「重點是操作方便程度，長按出現下面那一行操作
  介面，左滑有回覆功能」——原本以為是下載機制（blob vs 原生）的問題，
  實際上是訊息互動模型的問題：原本長按跳出的是貼著手指位置的小浮動
  選單（複製/回覆/編輯/刪除全部擠在一起），不是 Messenger 那種「表情
  反應貼近訊息＋其餘動作收在畫面底部一整排」的版型，而且完全沒有
  滑動手勢。
- **變更**（`assets/js/chat-thread.js`／`pages/chat/assets/css/
  page-chat.css`／`pages/chat-panel/assets/css/page-chat-panel.css`，
  兩份 CSS 各自維護一份是既有慣例）：
  - **長按選單拆成兩塊**：`openContextMenu()` 改成表情反應維持浮動
    貼在觸摸點附近（`els.contextMenu`，跟原本一樣），複製/回覆/編輯/
    刪除搬進新的 `els.actionSheet`——固定貼在畫面底部、圖示＋文字、
    背後有半透明背景點擊可關閉。兩個容器共用同一支
    `handleContextMenuClick()`（原本內嵌在 `els.contextMenu` 的
    click listener 抽出來變成具名函式，兩邊各自掛一份監聽），按鈕的
    `data-menu-*` 屬性跟實際動作完全沒變，只是換了容器/排版。
  - **新增左滑快速回覆**：跟長按共用同一個 `pointerdown` 起點，但
    追蹤用獨立的 `swipeStart`（不是 `longPressStart`，那個一超過
    10px 就被 `cancelLongPress()` 清空，滑動要撐到 `pointerup` 才算
    完）。方向鎖定一次（先看前 8px 是水平還是垂直位移大，垂直就整個
    放棄，不擋畫面本身的上下捲動）；水平且往左才即時位移訊息泡泡
    （`.jonaminz-chat-bubble-col` 套 `translateX`，夾在 -72px），
    單一共用的 ↩️ 圖示動態定位到當下滑動的訊息旁邊、透明度跟著滑動
    進度淡入；放開時位移超過 56px 才觸發 `setReplyTarget()`（跟長按
    選單的「回覆」按鈕是同一支函式）。**限制跟既有「回覆」按鈕一致**
    ：只有 `copyText` 存在的訊息（純文字，不含圖片/檔案/分享卡）才會
    啟動滑動追蹤——這是既有限制，不是這次新增的，圖片/檔案訊息本來
    就不能回覆。
- **狀態變化**：訊息長按/滑動的互動模型改版，跟使用者提供的參考
  截圖版型對齊。原本「電腦版下載體驗」那個問題還沒有直接回應——
  這次先處理的是操作方便程度本身，下載機制（blob fetch vs 原生
  連結）要不要改回去，還在等使用者更明確的回覆。
- **驗證**：`node --check` 通過。**完全沒有真人測試**——這是今天
  改動最大的一次手勢邏輯（新的水平/垂直方向鎖定＋`preventDefault()`
  時機），使用者現在人在現場可以馬上測，這次不用等下一輪才知道
  有沒有問題。
- **遺留**：(1) 使用者實測長按底部操作列跟左滑回覆是否符合預期
  （尤其要確認左滑不會誤觸發垂直捲動、也不會跟長按選單同時衝突
  出現）。(2)「電腦版下載」機制本身要不要改回原生連結，等使用者
  進一步說明。(3) 圖片仍不顯示／App 更新未生效兩項還在等使用者
  確認是不是快取問題造成的假性回報。
- **版本**：`v0.46.15-202607161422`。

---

## 2026-07-16（下午，第三筆）— 修角標雙重顯示；找到今天多起「沒看到變化」回報的可能共同根因（10 分鐘瀏覽器快取）

- **任務**：使用者真人測試一輪，回報好幾筆「完全沒有改變」——Chat
  圖片還是只顯示文字、原生 App 更新後泡泡收合功能沒出現、角標變成
  裡外雙重顯示。
- **變更**：
  - `pages/chat-launcher/assets/css/page-chat-launcher.css`——強制
    隱藏 `.jcl-presence`／`.jcl-badge`（`display:none!important`）。
    根因：2026-07-16 凌晨「角標搬出 iframe」那次改版，宿主端
    （`assets/js/chat-launcher.js`）新增了一份不會被裁切的角標/在線
    小綠點，但 iframe 內部（`pages/chat-launcher/index.html`）原本
    那份角標邏輯是刻意保留繼續跑（設計上假設反正會被 iframe 圓形
    裁掉看不見），這次真機證實裁切不可靠，兩邊同時可見。不能整支
    拿掉那份輪詢（`refresh()` 同時也決定大頭貼本體要不要顯示、畫
    哪個字母），只強制隱藏角標/小綠點這兩個視覺元素。
- **調查發現（不是這次改的程式碼，是找到的既有設計）**：
  `assets/js/entry-core.js` 2026-07-12 效能重建時，`version.js`
  本身刻意不帶 cache-buster、吃瀏覽器原生 `Cache-Control`——查證
  `curl -I https://www.jonaminz.com/version.js` 目前是
  `Cache-Control: max-age=600`（10 分鐘），這是當初就記錄在案、
  接受的取捨（「最壞下一次部署後 10 分鐘內看到舊版，可接受」）。
  這個設計假設是「使用者不會在 10 分鐘內連續測試同一批改動」——但
  今天這輪一直在快速修完就請使用者立刻測，很可能連續撞進這個 10
  分鐘視窗，導致「明明已經 push 了，畫面卻完全沒變」的假象，**很
  可能是今天好幾筆「沒看到變化」回報的共同根因**，不是每一筆都是
  獨立的程式 bug。原生 App 的懸浮泡泡面板另有一層更嚴重的版本：
  2026-07-15 就記錄過「長時間開著的 App 記憶體裡跑的還是網站上次
  載入當下的 JavaScript」，當時已經加了下拉重新整理功能解決，但
  使用者這次測試不確定有沒有用這個手法強制刷新。
- **狀態變化**：角標雙重顯示已修好。「圖片還是不顯示」「App 更新
  功能沒出現」這兩筆**還沒有動任何程式碼**，先回頭跟使用者確認
  是不是快取問題造成的假性回報，避免在正確的程式碼上瞎繞。
- **驗證**：CSS-only 變更，不用 `node --check`。
- **遺留**：(1) 使用者確認「圖片不顯示」在強制重新整理／清快取後
  是否還會重現。(2) 使用者確認原生 App 實際安裝的版本號是不是真的
  變成新版（設定 App 資訊應該看到 versionName `202607160839` 或更新，
  不是的話代表 debug 簽章更新在他的裝置上根本沒有真的安裝成功，
  不是功能沒生效，是整個新 APK 沒裝上去）。(3) 電腦版下載體驗
  「應該用原生下載」的回饋語意還不夠明確，需要使用者進一步說明
  具體想要什麼行為，不確定是不是要復原今天稍早的 blob 下載改動。
  (4) 使用者要求「三個月後回來看 OneDrive 空間使用量，評估要不要
  做一年封存對話紀錄」——記錄在此，不是這次要處理的項目，需要一個
  長期提醒機制（目前沒有排程機制，先寫進文件，之後開對話時人工
  記得回頭看，或考慮之後幫這個機制排一個 cron）。
- **版本**：`v0.46.14-202607161407`。
...
- **任務**：trim bug 修完、使用者重存 token 後，`tools/upload-apk.mjs`
  第一次就成功（`jonaminz-202607161326.apk`，`curl` 驗證過
  `/appDownload` 的 302 目標 `UniqueId` 對上這次上傳的 itemId）。
  使用者接著截圖回饋：點第一筆的「覆蓋」，表單卻出現在整份清單最
  下面（第四筆卡片下方），完全不像在編輯同一列。
- **變更**（`pages/admin/secrets/`，純前端，不用 wrangler deploy）：
  「覆蓋」不再共用「+ 新增密鑰」那個釘死在清單最下面的表單，改成
  在**被點的那張卡片正下方**動態插入一個只問「新的值」的小表單
  （名稱已知，不用再問）——`insertAdjacentHTML("afterend", ...)`
  插進 `.jonaminz-secret-list` 的 flex 子層，天然吃到既有間距，左邊
  加一條主色邊條視覺上跟上面的卡片連在一起。「+ 新增密鑰」表單維持
  在最下面，只用來新增全新項目（要問名稱＋值）；兩種表單同時只能
  開一個，開一種前會先關掉另一種。
- **另**：這輪對話中我不慎執行了一個誤刪指令，刪掉使用者剛上傳給我
  的截圖檔案（`~/.claude/uploads/.../*.jpeg`）——內容當下已經讀取
  完成、沒有資訊遺失，但檔案本體確實被刪除，當場向使用者說明並
  道歉，記錄在此避免被誤以為沒發生過。
- **狀態變化**：APK 上傳流程（agent 專用密鑰＋trim 修復）端到端驗證
  成功；保管箱「覆蓋」的操作體感修正為原地編輯。
- **驗證**：`node --check` 通過。**沒有瀏覽器實測**這次的 UI 改動，
  請使用者實際點一次「覆蓋」確認表單真的出現在同一列下面。
- **遺留**：無。
- **版本**：`v0.46.12-202607161330`。
...
- **任務**：使用者拿著手機即時測試稍早幾筆修復時，在 `for_claude` 待辦板
  新回報兩筆問題：(1)「電腦版chat可以成功下載檔案，但是手機版不行」
  (2)「Chat圖片，如果是圖片格式是不是要縮圖顯示加上可以顯示不用直接
  下載」。
- **變更**（`assets/js/chat-thread.js`）：
  1. **檔案下載改用 blob 觸發**：原本用 `<a download>` 觸發同分頁下載，
     `download` 屬性對跨網域連結（Graph 的 downloadUrl 跟 jonaminz.com
     不同源）桌機瀏覽器大多還是會忽略屬性、當普通連結開啟＋順便觸發
     下載，但手機瀏覽器（尤其 iOS Safari／App 內建 WebView）更嚴格，
     常常整個不動作——這正是桌機能下載、手機不能下載的原因。改成先
     `fetch()` 成 blob 再用 `blob:` URL（同源）觸發 `download`，所有
     平台都可靠；只有 fetch 失敗（例如 downloadUrl 不允許跨網域 fetch）
     才退回舊的開新分頁方式頂著。
  2. **選錯按鈕上傳的圖片改內顯示**：「選檔案」（跟「選圖片」是composer
     上兩個不同按鈕）在手機上系統選擇器常常直接秀相簿，容易選錯按鈕，
     圖片因此被存成 `kind:'file'` 走下載卡片，看不到內容。
     `requestFileUpload` 上傳當下其實已經有存 `mimeType`（`sendFile()`
     的 `file.type`），只是原本沒有拿來畫面。不改上傳流程本身（硬塞
     `kind:'image'` 會少了 w/h／模糊縮圖等圖片專屬欄位），純粹加一個
     渲染分支：`kind==='file'` 但 `metadata.mimeType` 是 `image/*`，沿用
     圖片泡泡的畫法（含點擊放大看原圖），不用引導使用者重傳一次。
- **驗證**：`node --check` 通過。**沒有真機測試**——這兩筆改動範圍小
  （檔案下載改觸發方式、圖片渲染多一個條件分支），風險比今天稍早的
  iframe 角標搬遷小很多，但終究沒有人在手機上點過。
- **版本**：`v0.46.5-202607160843`。
...
- **任務**：使用者對「泡泡角標還是被切到」那筆回「做吧做吧」——授權
  真的動手做「把角標搬出 iframe」這個之前判斷份量較大、先前只做了
  「復原 border-radius」治標版本的根本解法。使用者當時已經說「你做吧
  我要睡覺了，你慢慢把剩下的等做完」，這輪起自主完成不逐步確認。
- **變更（真正的修法）**：`assets/js/chat-launcher.js`／
  `sdk/sdk-src/sdk.js`——未讀角標／在線小綠點改成宿主自己文件裡的
  獨立元素（`<span>`），不再放 iframe 內部被裁切，這是唯一不受「這個
  瀏覽器對 iframe 的 clip-path 不生效」限制影響的做法。位置對齊大頭貼
  圓形的右上/右下角（一半疊在圓上一半露出來，跟 FB 角標同一種畫法），
  跟著大頭貼一起拖動/吸邊/鎖定/還原。資料來源獨立輪詢
  `listChatMessages`（每 4 秒），跟 `pages/chat-launcher/index.html`
  內部原本的算法完全一致，刻意重複一份。**`sdk-src/sdk.js` 這份因為
  是外部網域宿主頁面、跟 jonaminz.com 不同源，沒辦法讀到
  `jonaminz.sessionToken`**（這是 `assets/js/chat-launcher.js` 可以
  這樣做的前提——那邊宿主頁面本身就是 jonaminz.com 同源），沒有查到
  現成的跨網域換 token 管道，改成安全的 no-op（`token=null` 直接不
  打），角標對外部專案暫時不會顯示——不影響任何正式功能，目前也沒有
  任何外部專案真的被授權 `chat.launcher@1` 這個 capability。
- **意外抓到一個我自己造成的嚴重 bug（`assets/js/chat-launcher.js`）**：
  審查時發現稍早（第七十三次）加的「換頁保留拖動位置」那段
  `if (freeLeft !== null) applyPosition(freeLeft, freeTop);`，被放在
  `overlay`／`launcherFrame` 之後創建的**元素還沒被建立之前**就呼叫
  ——`applyPosition()` 內部直接存取這些元素的 `.style`，對還不存在的
  變數賦值會丟出 `TypeError`，而這個檔案的 `mountChat()` 整支函式沒有
  外層 try/catch 包住，這個例外會讓**這行以後所有初始化（overlay、
  拖動手勢、面板開關、手勢排除區同步）全部沒執行到**，等於只要使用者
  之前拖過泡泡一次（`freeLeft` 不是 null），之後每次換頁泡泡就完全
  故障、不能點、不能拖。已經搬到所有相關元素都建立完成之後才呼叫。
  **這條 bug 存在於稍早已經上線的 commit，代表使用者這幾小時如果剛好
  拖過泡泡，可能已經實際遇到泡泡故障**——請務必實測一次拖動＋換頁＋
  再拖動，確認現在正常。
- **驗證**：`node --check` 兩份都通過；`sdk-src/sdk.js` 改動後重新
  產生 immutable 檔 `sdk-035062b5de91.js`，`sdk-versions.json`
  revision 23，`wrangler deploy --dry-run` 通過後部署（Worker Version
  `25dc8682`）。**沒有 Playwright 可用，這次修法（尤其角標定位偏移量
  的視覺效果）完全沒有截圖驗證過**，是這一整晚風險最高的一次改動，
  純靠程式碼審查抓到那個嚴重 bug，不代表沒有其他沒發現的問題。
- **狀態變化**：角標/小綠點被切到的問題（第十三次修正遺留至今）終於
  真正解決，不再是「復原成舊的部分正確狀態」。
- **遺留**：**強烈建議使用者醒來後第一件事就是實測泡泡拖動**——這條
  意外抓到的 bug 風險等級最高，比角標好不好看重要得多。
- **版本**：v0.46.2-202607160108

## 2026-07-16（凌晨，第七十八次）— 後台首頁「有什麼新的」摘要列

- **任務**：使用者對決策圖候選「後台首頁摘要卡片」直接說「你做吧我要
  睡覺了，你慢慢把剩下的等做完」——本輪最後一筆待辦，授權自主完成
  不用逐步確認。
- **變更**：`pages/admin/assets/js/app.js`——後台首頁問候語下方新增
  「有什麼新的」摘要列，三個可點擊連結：
  1. 未讀訊息數（連去 `/pages/chat/`）——算法直接照抄
     `pages/chat-launcher/index.html` 既有的未讀角標邏輯（比對
     `lastReadMessageId` 之後、對方傳的訊息），刻意重複一份，理由跟
     這個專案裡其他 shell script 各自維護一份小工具一樣，不是想偷懶
     共用就共用。
  2. 待你驗證的任務數（連去 `/pages/admin/journal/`）——定義是
     `lane==='for_user' && origin==='claude' && !done`，跟決策與待辦
     頁「請驗證：」的慣例是同一批東西，不是泛指所有未完成待辦（使用者
     自己打的待辦只是待處理，不算「待驗證」）。
  3. OneDrive 連接狀態（連去 `/pages/admin/connections/`）——兩人都
     連接／只有一人／都沒連接三種文字。
  三個資料來源都是既有 action（`listChatMessages`／`listProjectTasks`
  ／`getOnedriveStatus`）前端聚合，不動 `worker.js`；跟既有的 pending
  數量／registrations 同一個原則，讀取失敗只在各自區塊顯示文字，不擋
  頁面本身的 all-ready。`page-admin.css` 新增對應樣式，刻意做成低調的
  小長條（不跟入口卡片搶視覺重量，「安靜的家」版型入口卡片才是主角）。
- **驗證**：`node --check` 通過；起臨時 static server 確認頁面/JS/CSS
  都能載入（200）。**沒有**用真實登入 session 在瀏覽器裡跑完整流程
  （沒有 Playwright 可用），三個摘要的資料邏輯是照抄既有已驗證過的
  算法組合而成，風險相對低，但仍需要使用者自己確認數字對不對。
- **狀態變化**：決策圖候選「後台首頁摘要卡片」畢業成真正上線的功能。
- **遺留**：等使用者實際看一次三個數字準不準（尤其未讀數，跟浮動大
  頭貼角標比對是否一致）。
- **版本**：v0.46.1-202607160050
