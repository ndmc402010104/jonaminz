...
6. **修改後更新交接文件**：會改變專案狀態的任務，完成後更新
   ...
   **這份文件是累積性知識庫，要讓它一直長大，不是寫完這輪就算了**
   ...
   是什麼」，不能依賴聊天記錄。
   **同一份精神也適用於「決策與待辦」頁的 `DECISION_TIMELINE`**
   〔使用者，2026-07-15 定案〕：CHANGELOG 記的是工程視角的逐筆流水帳，
   `pages/admin/journal/assets/js/app.js` 的 `DECISION_TIMELINE` 陣列是
   給人看的「精選重大決策」時間軸——**任務開始前跟結束後都要看一眼這個
   陣列**，開始前用它快速理解專案脈絡，結束後如果這次任務做了「有取捨、
   值得記住為什麼這樣決定」的重大決策，就手動加一筆（格式比照既有條目：
   情境／考慮的選項／決定與理由／現況）。這條被使用者當面抓到——連續
   做完好幾輪明顯夠格的決策（OneDrive Phase 上線、待辦板權限重新設計）
   都沒有更新這個陣列，只寫了 CHANGELOG，不能只顧著寫工程紀錄漏了給人看
   的那份。不是每個小修都要加，只有真正的決策。
