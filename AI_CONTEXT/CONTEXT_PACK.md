...
<!-- generatedAt: 2026-07-13T10:50:05.925Z -->
...
…（已截斷，`AI_CONTEXT/FACTS.md` 原始內容共 12674 字元，完整內容請直接讀取該檔案）
...
13. **AI 呼叫 App 能力時仍需經過平台治理**（不能繞過 Contract／Integration
    ...
    這條裁決暫時無法用程式碼驗證，只能記錄方向本身與現有 `identity.current-user@1`
...
…（已截斷，`AI_CONTEXT/DECISIONS.md` 原始內容共 12715 字元，完整內容請直接讀取該檔案）
...
…（已截斷，`AI_CONTEXT/CURRENT_STATE.md` 原始內容共 7078 字元，完整內容請直接讀取該檔案）
...
…（已截斷，`AI_CONTEXT/KNOWN_ISSUES.md` 原始內容共 4899 字元，完整內容請直接讀取該檔案）
...
…（已截斷，`AI_CONTEXT/CHECKPOINTS.md` 原始內容共 5809 字元，完整內容請直接讀取該檔案）
