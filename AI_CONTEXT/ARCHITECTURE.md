...
最後更新：2026-07-13（修正兩處 2026-07-11 遺留的過期敘述：approve/reject
的保護機制已改用登入 session、`capabilities` 已是真實交集非佔位空陣列；
補 §7 圖書館模型視覺分流待辦）
...
approve/reject 已實作並上線（`/pages/admin/contracts/` 後台，登入保護——
`JONAMINZ_ADMIN_TOKEN` 那把密語已於 2026-07-12 完全淘汰，改用
`requireSession()`，見 `AI_CONTEXT/FACTS.md` #15、`CURRENT_STATE.md`。
本段原文寫的 Worker secret 保護是本文最後更新（2026-07-11）當時的舊
機制，2026-07-13 盤點時已過期，這裡是唯一修正處，本文其餘內容仍屬
現況）：`listPendingContracts`（公開唯讀，2026-07-13 起 `previousApproved`
另外帶 `origin` 欄位，來自 `integration-settings.json` 的伺服器端登記
資料，供前端安全組出外部專案的「進入」連結，見 `pages/admin/design/`）／
...
v1 只有 none/tokens 兩級）。`capabilities` 自 implementation plan 第 9
項階段 B（2026-07-12）起是真實交集運算（`resolveEffectiveCapabilities`：
...
S31/S38，算「approved Contract × Settings 授予」，`css`／`capabilities`
兩個維度都是真實值，見上面 §4）、SDK Loader（`sdk/jonaminz-entry.js`
常青 loader＋`getSdkVersion` 版本指標，S37）、SDK Kernel
（`sdk/sdk-src/sdk.js`，S18-23/S26-29，真的做 contract discovery、
...
- **視覺架構的圖書館模型分流**（`DECISIONS.md` §四，2026-07-13）：
  公開圖書館／登入後管理員室的視覺分流機制尚未實作，現況與最小遷移
  方案見 `EXPERIMENTS.md` #10。
