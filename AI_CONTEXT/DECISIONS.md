...
## 六、Minz 的房間：Minz Page v0.1 資訊架構（2026-07-13 使用者裁決）

使用者準備了一份完整的 Minz 個人頁面規格（`docs/minz-page/README.md`
忠實重整版），裁決採用其中的產品定位與資訊架構原則。這是 §四第 18-21
條「大廳／房間／後台」裡「Jonathan、Minz 各自獨立房間」的 Minz 那一半
具體落地方向——目前**完全尚未實作**，只是方向與架構原則裁決。
...
27. **Minz 頁面跟「技術方案」（未來的 Travel 等內容管理系統，呼應
    ...
    這類 Pack Type）是兩個東西，責任邊界明確：** 技術方案負責完整
    旅程規劃、實際走訪、票券／住宿等私人資料、PDF 輸出等「不輕鬆的
    技術後台流程」；Minz 頁面只保存展示用的來源引用
    （`sourceProject`／`sourceId`／`sourceChapterId` 等）與展示
    metadata（顯示標題、封面、分類、排序、授權範圍），**永遠不複製
    完整遊記內容**。這條邊界是這份規格裡最重要的架構決策，任何實作都
    不可違反。**實作狀態：技術方案本身也還沒開始（不同專案／不同
    範圍），Minz 頁面自然也還沒有真的資料可以引用，第一階段必須先用
    mock data 驗證，不能假裝技術方案已經存在。**
...
29. **隱私與可見度分級是實作必要項，不是事後補強：** `public`／
    ...
    內容/照片；`noindex` 明確**不是**安全機制，只是搜尋引擎收錄設定，
    真正的存取控制必須在資料層/後端判斷，不能只靠前端 CSS/JS 隱藏或
    不連結。所有新頁面預設 `noindex`，sitemap 只收錄明確允許收錄的
    已公開頁面。**實作狀態：完全未實作，但這是後續任何一行程式碼都
    要遵守的安全底線，不可為了省事先做「看起來對」的前端隱藏就跳過
    真正的授權判斷。**

30. **後台策展體驗必須是 Minz 能獨立在手機上完成的日常操作**（預覽→
    確認或微調→發布），**不需要 Jonathan 協助日常維護**，也不需要
    使用者具備設計或寫程式能力。**實作狀態：未實作。**

31. **第一階段明確排除的範圍**（跟 ADPF §9 同樣的收斂精神，避免下一個
