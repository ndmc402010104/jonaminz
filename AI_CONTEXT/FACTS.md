...
| 17 | `identity.current-user@1`（capability ID，2026-07-14 由 `identity.currentUser@1` 改名——原本的 camelCase 撞上 contract schema 的 kebab-case pattern，外部專案永遠無法合法宣告它，見 `KNOWN_ISSUES.md` #12，已修復並 `wrangler deploy`）機制已在程式碼與正式環境就位，但**目前沒有任何專案被授權使用**。函式名 `window.Jonaminz.identity.currentUser()` 不受改名影響 | `backend/cloudflare-worker/integration-settings.json`（movies＋travel 兩筆；travel 已授權 `chat.launcher@1`，identity 仍無人授權）；`worker.js` `resolveEffectiveCapabilities()`／`getGrantedIdentity` 邏輯已完整存在 | 是 | 是（`wrangler deploy` 後 SDK release `407d53fc5d80`，`sdk-versions.json` revision 8） | 部分：機制本身的六種情境（未授權/已授權+已登入/已授權+未登入/雙重防線/逾時/偽造來源）已用 Playwright 對 mock+真實 Worker 混合環境驗證過（CHANGELOG 記載）；「正向授權」路徑現在 schema 上已可行，但仍沒有真實專案宣告支援這個 capability，尚無真實資料可測 |
