...
| 17 | `identity.currentUser@1` capability 機制已在程式碼與正式環境就位，但**目前沒有任何專案被授權使用**。**2026-07-14 補充**：正向授權路徑其實被 schema 擋死——`identity.currentUser@1` 是 camelCase，不符合 contract schema 的 kebab-case capabilityId pattern，外部專案永遠無法在合約裡合法宣告它，見 `KNOWN_ISSUES.md` #12（同日新增的 `chat.launcher@1` 已刻意取 kebab-case，不受影響） | `backend/cloudflare-worker/integration-settings.json`（2026-07-14 起有 movies＋travel 兩筆；travel 已授權 `chat.launcher@1`，identity 仍無人授權）；`worker.js` `resolveEffectiveCapabilities()`／`getGrantedIdentity` 邏輯已完整存在 | 是 | 是（CHANGELOG 記載已 `wrangler deploy`，`getSdkVersion` 指向含此 capability 的 SDK hash `5d8e909081bf`；2026-07-14 之後見 CHANGELOG 新條目） | 部分：機制本身的六種情境（未授權/已授權+已登入/已授權+未登入/雙重防線/逾時/偽造來源）已用 Playwright 對 mock+真實 Worker 混合環境驗證過（CHANGELOG 記載）；「正向授權」如左欄補充所述，在 schema 修正前結構上不可能有真實資料 |
...
| 27 | Movies（`jonaminz-movies`）是獨立 repo、獨立部署（GitHub Pages）的 first-party 外部專案，不是 jonaminz Core 內部模組（**2026-07-14 更新**：`jonaminz-travel` 是第二個同模式的外部專案，同日並成為第一個真的嵌入 SDK snippet 的專案） | `integration-settings.json` 登記 `jonaminz-movies`＋`jonaminz-travel` 兩筆，origin 皆 `https://ndmc402010104.github.io`；獨立 repo `ndmc402010104/jonaminz-movies`／`jonaminz-travel` | 是 | 是（兩 repo 皆已公開部署） | 是（movies/travel 各自跑過完整 submit→approve 流程） |
| 28 | SKHPSv2 定位是外部接入 App（透過 Contract／identity capability 機制），不是 jonaminz Core 的一部分；SKHPSv2 尚未透過 Contract 機制正式登記（**2026-07-14 更新**：`integration-settings.json` 現有 movies＋travel 兩筆，仍不含 SKHPSv2；且 identity capability 有 KNOWN_ISSUES #12 的 schema 命名 bug 要先修才可能真的接） | `integration-settings.json`；`docs/platform-integration-v1-implementation-plan.md` 明文「SKHPSv2 正式接入 jonaminz……2026-07-11 明確裁決不急」 | 是（尚未登記是事實，不是遺漏） | — | — |
