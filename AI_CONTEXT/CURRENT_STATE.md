...
| 視覺架構 | 大廳（首頁）已改成「米紙 Rice Paper」＋真的圓相字標（`--color-bg-lobby`／`--color-text-lobby`／`--color-lobby-accent`，2026-07-13 已實作並驗證）；Minz 的房間 Phase 1 骨架已實作（米白×墨綠×淡棕，2026-07-13）；後台仍是「亞麻米 Flax & Ink」，其中**後台首頁**（`/pages/admin/`）2026-07-13 已從置中小卡片改版成「安靜的家」（迎接區＋依待審數量動態放大的入口卡＋安靜清單），其餘管理子頁（theme/contracts/design）維持原樣未動；Jonathan 的房間**尚未實作**（另有一份 2026-07-13 收到的「Dark Precision 精密展示艙」完整規格待實作，見 `PROJECT_STATE.md`）；`theme_css_rules` 單一 `unique(selector,property)`，無 space 概念 | 大廳／房間／後台三層（`DECISIONS.md` §四）：Jonathan 的房間＝深夜訊號黑色系／Dark Precision（主色與細節待實作）；分流機制（讓 Theme 後台能分空間管理，不用再手動改 CSS 檔）見 `EXPERIMENTS.md` #10＋`DECISIONS.md` §五 ADPF，**尚未實作**；細粒度 Placement（每個展示元件獨立控制顯示/排序/區域/釘選）**確認不存在，尚無資料模型**，使用者已澄清不急著建 |
