...
## 12. ~~`identity.currentUser@1` 的 capability ID 撞上 schema 的 kebab-case 限制~~ 已修復（2026-07-14）

**修復**：使用者拍板選了「改名」這條路。capability ID 全面改成
`identity.current-user@1`（kebab-case，通過 schema pattern）——
`worker.js`（`getGrantedIdentity` 的檢查字串）、`sdk-src/sdk.js`
（`IDENTITY_CAPABILITY` 常數與兩處錯誤訊息字串）已改，重新產生 SDK
release `407d53fc5d80`，`sdk-versions.json` revision 8 指過去。
`window.Jonaminz.identity.currentUser()` 這個函式名沒有變——S30 的
capability ID 命名跟 S32 的 API 物件函式命名是兩個獨立維度。
`integration-settings.json` 沒有任何專案宣告過舊值（本來就過不了
schema），不需要資料遷移。已 `wrangler deploy`，curl 驗證
`getSdkVersion` 指標與 bundle 語法皆正確。

以下為原始發現紀錄（2026-07-14 發現），保留供歷史脈絡參考：

## ~~原標題~~ `identity.currentUser@1` 的 capability ID 撞上 schema 的 kebab-case 限制，外部專案永遠無法在合約裡合法宣告它

**現況**：`docs/contract-schema/jonaminz.contract.schema.json` 的
`$defs/capabilityId` pattern 強制 kebab-case（schema 自己的說明文字
明講「不允許 camelCase」，並稱定案前是「最後一次免費改名機會」），但
第 9 項階段 B 實際發布的 capability ID 是 **camelCase 的
`identity.currentUser@1`**（`worker.js` `resolveEffectiveCapabilities`／
`getGrantedIdentity`、`sdk-src/sdk.js`、`integration-settings.json`
的授權值全部用這個字串）。已用 Node 對 schema pattern 實測：
`identity.currentUser@1` **不通過**、`chat.launcher@1` 通過。

**後果**：任何外部專案想在 `jonaminz.contract.json` 的
`capabilities.supports` 宣告 `identity.currentUser@1`，schema 驗證這關
就會把整份合約判 invalid，`submitContract` 直接拒收——S31 的交集公式
（Contract supports ∩ Settings 授權）永遠得不到這個 capability，
「正向授權路徑」在結構上不可能發生。這也正好解釋了為什麼階段 B 的
正向授權當時只能在 mock 環境驗證（當時的紀錄寫「沒有真實資料可測」，
真相是真實資料根本過不了 schema）。

**為什麼還沒炸**：至今沒有任何專案真的要用 identity capability
（skhpsv2 接入未開始），零消費者，所以零實際影響。

**修法選項（未裁決，需要使用者拍板）**：
- 改名 capability 為 `identity.current-user@1`（kebab-case）——需要
  同步改 worker.js／sdk.js（重新發 release）／integration-settings.json
  三處＋wrangler deploy。`window.Jonaminz.identity.currentUser()` 這個
  **函式名**不用動（S32 管的是 API 物件上的函式，capability ID 是另一
  個維度的識別字串）。因為零消費者，現在改名成本最低，拖越久越貴。
- 或放寬 schema pattern 允許 camelCase——但 schema 說明文字明確記載
  kebab-case 是刻意的定案決策，放寬等於推翻當時的裁決，且 schema 改了
  要重跑 `generate-contract-validator.mjs` 並重新部署。

**風險等級**：中（不影響現有任何功能，但擋死一條規格明文存在的路，
且知道的人不修、之後接手的人會在 skhpsv2 接入時撞牆）。

**驗證**：`node -e` 直接用 schema 的 regex 測兩個字串（2026-07-14，
見 CHANGELOG 同日條目）；`chat.launcher@1`（同日新增的第二個
capability）已刻意取 kebab-case 命名，不受此問題影響。
