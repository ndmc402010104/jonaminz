...
- **2026-07-14 下午（P2＋連續五輪修正後的最終架構）**：DECISIONS.md
  ...
  - **第十輪（同日）**：(1) 預設休息位置（jonaminz 內部頁面專屬，外部
    專案不變）從右下角改成頁首正下方（`computeRestTop()` 讀
    `JonaminzLayoutMetrics` 的 header 高度），避免壓到首頁本來就有的
    「Jonathan」切換身分連結；開啟面板時鎖到的位置維持右下角不變。
    (2) `setPanelOpen()` 加幂等防呆＋點外關閉加 300ms debounce，避免
    連續快速點擊造成狀態卡住。(3) `pages/chat-launcher/` 的未讀角標／
    在線小綠點原本貼著方形文件角落，但外層 iframe 被裁成正圓，角標
    外側一角一直被裁掉——內縮到 `12px` 修正（算過對角距離，留了
    緩衝）。
