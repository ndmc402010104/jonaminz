...
分類資料仍是 mock data（技術方案專案尚未開始）。**後台首頁**（登入後
畫面）已從「置中小卡片」改版成「安靜的家」：迎接區＋依待審數量動態
放大的入口卡（不是統一格柵）＋安靜清單，並補上原本完全沒有頁面連過去
的孤兒頁面 `/pages/admin/design/` 入口，`v0.21.10`，見同日 CHANGELOG。
**Jonathan 的房間 v1 已實作**（`docs/jonathan-page/README.md`／
`DECISIONS.md` §七，「Dark Precision 深色精密工作室」）：Hero 左右
分欄＋純 CSS 合成的抽象精密展示艙（金屬環／深色玻璃／精密切線／呼吸光
核心／抽象 UI 碎片，無真實 SKHPSv2 截圖）＋SKHPSv2 registry-driven
詳細卡＋獨立 About 子頁（`pages/jonathan/about/`，新增頁面），
Playwright 驗證桌機／窄桌機／手機／reduced-motion 四種情境皆通過，
8 頁回歸無波及，`v0.21.11`。大廳／Jonathan／Minz／後台四個視覺方向
至此全部完成第一輪實作，下一步是使用者裁決要不要進入 Theme Studio
階段（見 `EXPERIMENTS.md` #10、`DECISIONS.md` §五 ADPF）。
...
- **2026-07-14 下午（P2＋連續五輪修正後的最終架構）**：DECISIONS.md
  ...
  - **第七輪（同日）新增行為**：(1) 大頭貼可以自由拖動；(2) 面板一開始
    ...
    最新的，不會有「載入中」的空檔。
  - **第八輪（同日，拖動微調）**：放開拖曳會依水平中點吸到最近的左或右
    邊緣（`snapToNearestEdge()`，像 iOS AssistiveTouch），不是停在任意
    放開位置；面板開著時大頭貼鎖在固定角落、`pointermove` 直接忽略、
    完全不能拖動；關閉面板時還原回「開啟前」的休息位置
  ...
  - **第十輪（同日）**：(1) 預設休息位置（jonaminz 內部頁面專屬，外部
    專案不變）從右下角改成頁首正下方，避免壓到首頁本來就有的
    「Jonathan」切換身分連結（第十一輪之後展開時的位置也統一過去，見
    下方）。(2) `setPanelOpen()` 加幂等防呆＋點外關閉加 300ms debounce，
    避免連續快速點擊造成狀態卡住。(3) `pages/chat-launcher/` 的未讀
    角標／在線小綠點原本貼著方形文件角落，但外層 iframe 被裁成正圓，
    角標外側一角一直被裁掉——內縮到 `12px` 修正（算過對角距離，留了
    緩衝）。
  - **第十一輪（同日，現在的最終狀態）**：收合休息位置跟展開鎖定位置
    統一成同一個地方（都在頁首正下方），不再分成兩個錨點——改用 CSS
    自訂屬性 `--jcl-anchor-top` 存高度，大頭貼／覆蓋層／面板的定位都靠
    `calc(var(--jcl-anchor-top) + ...)`，`JonaminzLayoutMetrics` 更新時
    只要改這一個變數三者自動跟著動。面板本身也從「往上展開」改成
    「往大頭貼下方展開」（大頭貼現在在頁首下方，面板自然往下長到接近
