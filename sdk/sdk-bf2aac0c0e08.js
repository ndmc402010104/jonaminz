...
(function () {
  ...
  // pages/chat-launcher/，按鈕本體/未讀 polling 都在那頁裡面跑——外部
  // 專案這裡讀不到 jonaminz 的 session token，也不需要讀。點擊時 embed
  // 頁 postMessage 過來，這裡驗 event.origin（embed 頁自己刻意不驗，
  // 跟 identity-relay 同一個分工：訊息內容無敏感資料，該把關的是接收端）。
  function mountChatLauncher() {
    try {
      ...
      window.addEventListener("message", function (event) {
        ...
        if (data.action === "open") window.location.href = CHAT_PAGE_URL;
      });
