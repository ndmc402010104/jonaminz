...
實作上是注入一個小 iframe 指到 `pages/chat-launcher/`（跟 identity-relay
同一個「同源小頁面」模式）：按鈕本體、未讀數 polling、在線狀態全部住在
jonaminz.com 網域裡的那頁（外部專案讀不到 jonaminz 的 session token，
也保證按鈕長相是同一份程式碼、宿主頁面的 CSS 碰不進 iframe）。點擊時
embed 頁 postMessage 過來，這裡驗過 event.origin 才導頁去完整聊天頁。
...
(function () {
  ...
  var CHAT_PAGE_URL = "https://www.jonaminz.com/pages/chat/";
  ...
  // 不發布任何 window.Jonaminz.* API）。iframe 指到 jonaminz.com 的
  // pages/chat-launcher/，按鈕/面板本體/未讀 polling 都在那頁裡面跑——
  // 外部專案這裡讀不到 jonaminz 的 session token，也不需要讀。
  //
  // 2026-07-14（P2）：從「固定 76x76、點擊導頁」改成三態懸浮面板
  // （收合/半版/全版）——embed 頁只 postMessage 目前狀態字串，實際
  // 尺寸由這裡的 CSS 決定（vw/dvh 相對單位，交給宿主自己的瀏覽器
  // 引擎算，不是 embed 頁自己在 iframe 內猜宿主視窗多大）。
  // event.origin 驗證在這裡做（embed 頁自己刻意不驗，跟 identity-relay
  // 同一個分工：訊息內容無敏感資料，該把關的是接收端）。尺寸數字跟
  // assets/js/chat-launcher.js 是刻意重複的兩份（同一個理由：這批
  // shell script/SDK 注入器彼此獨立，不互相依賴）。
  //
  // 2026-07-14（同日第二次修正）：使用者在真實 Android 裝置上回報
  // 「圓形外面卡一個方框」，且比早上那次（以為是 box-shadow 被裁到、
  // 加大邊距）還嚴重——真正原因是 iframe 內部的透明背景在那台裝置的
  // 瀏覽器/WebView 上不可靠，不是邊距大小問題。改成**不依賴 iframe
  // 內部透明**：`border-radius`／`box-shadow` 直接套在這裡建立的
  // `<iframe>` 元素本身（收合態裁成圓形、展開態裁成圓角矩形）——這是
  // 瀏覽器對「一般元素」的裁形狀與陰影繪製能力，跟這個元素裡面的文件
  // 透不透明完全無關。embed 頁那邊對應改成兩個視圖都完整填滿 100%、
  // 不留任何透明空白，見 pages/chat-launcher/index.html 檔頭。
  function mountChatLauncher() {
    try {
      ...
      iframe.src = CHAT_LAUNCHER_EMBED_URL;
      iframe.title = "Jonaminz Chat";
      ...
      var append = function () { document.body.appendChild(iframe); };
