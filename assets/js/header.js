...
(function () {
  ...
  // 2026-07-14 補記：全站浮動 Chat 入口原本寫在這裡（mountChatBubble），
  // 同日拆去獨立的 shell script assets/js/chat-launcher.js＋
  // pages/chat-launcher/（iframe 模式）——「品牌列/身分區塊」跟「Chat
  // 入口」是兩個不相關的職責，不該糾在同一個檔案，且 iframe 模式讓
  // 內部頁面跟外部專案（SDK chat.launcher@1）共用同一份按鈕實作。
