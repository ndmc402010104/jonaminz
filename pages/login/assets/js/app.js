...
(function () {
  ...
  function googleStartUrl() {
    ...
    // 帶自己的網域給 Worker，登入完成後才知道要導回正式站還是本機
    // localhost:5500（見 worker.js 的 ALLOWED_OAUTH_RETURN_ORIGINS 白
    // 名單——Worker 只認白名單裡的值，這裡帶什麼字串都不會變成漏洞）。
    return baseUrl.replace(/\/+$/, "") + "/auth/google/start?origin=" + encodeURIComponent(window.location.origin);
  }
