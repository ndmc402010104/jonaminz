...
(function () {
  ...
  // Claude 接手，原項目隨之移除（不是複製兩份）。
  // 「你想叫我做的」(for_claude)：使用者自由新增的點子清單，不想要了
  // 就直接 ✕ 刪除，這裡刪除是合理的動作。

  var currentToken = null;

  function taskItemHtml(lane, task) {
    var actionHtml = lane === "for_user"
    ...
    var escalateFormHtml = lane === "for_user"
      ? '<form class="jonaminz-journal-escalate" data-escalate-form hidden>' +
          '<input type="text" placeholder="說明需要怎麼調整..." data-escalate-input>' +
          '<button type="submit">轉給 Claude</button>' +
          '<button type="button" data-escalate-cancel>取消</button>' +
        '</form>'
      : "";
    return (
      '<li class="jonaminz-journal-task' + (task.done ? " is-done" : "") + '" data-task-id="' + escapeHtml(task.id) + '" data-task-text="' + escapeHtml(task.text) + '">' +
  ...
  function renderLane(lane, rows) {
    ...
    listRoot.innerHTML = active.length
      ? active.map(function (row) { return taskItemHtml(lane, row); }).join("")
      : '<li class="jonaminz-journal-empty">目前沒有項目。</li>';
    doneRoot.innerHTML = done.map(function (row) { return taskItemHtml(lane, row); }).join("");
  ...
  function bindLaneEvents(lane) {
    ...
    // 每次 loadTasks() 都會整個重繪，個別綁定會失效，代理在外層容器上
    // 就不用每次重繪後重新綁定）。只在第一次呼叫時掛一次。
    ...
    board.addEventListener("click", function (event) {
      if (event.target.matches("[data-task-delete]")) {
        ...
        if (!deleteLi) return;
        window.JonaminzBackend.deleteProjectTask({ token: currentToken, id: deleteLi.dataset.taskId })
          .then(loadTasks)
          .catch(function (error) {
      ...
      if (event.target.matches("[data-task-escalate]")) {
        ...
        if (escalateForm) escalateForm.hidden = !escalateForm.hidden;
        return;
      }
      if (event.target.matches("[data-escalate-cancel]")) {
        var cancelLi = event.target.closest("[data-task-id]");
        if (!cancelLi) return;
        var cancelForm = cancelLi.querySelector("[data-escalate-form]");
        if (cancelForm) cancelForm.hidden = true;
        return;
      }
    });

    // 「›」的轉送表單：把原文字＋使用者補充的調整說明，變成一筆新的
    // for_claude 項目，原本 for_user 那筆隨之刪除（移過去，不是複製）。
    board.addEventListener("submit", function (event) {
      ...
      var originalText = li.dataset.taskText;
      var newText = note ? (originalText + "：" + note) : originalText;
      window.JonaminzBackend.addProjectTask({ token: currentToken, lane: "for_claude", text: newText })
        .then(function () {
          return window.JonaminzBackend.deleteProjectTask({ token: currentToken, id: li.dataset.taskId });
        })
        .then(loadTasks)
        .catch(function (error) {
