...
(function () {
  ...
  // 舊的）。
  var DECISION_TIMELINE_UPDATED_AT = "2026-07-15T21:37:00+08:00";
  ...
  // 記得更新下面這個時間戳。
  var DECISION_MAP_UPDATED_AT = "2026-07-15T20:44:00+08:00";
  ...
  var DECISION_MAP = [
    ...
    {
      id: "admin-home-summary",
      title: "後台首頁摘要卡片",
      summary: "後台首頁目前只是連結列表，加一塊「有什麼新的」摘要：未讀訊息數、待驗證任務數、OneDrive 連接狀態。",
      why: "這些資料來源都已經存在（Chat 未讀數、project_tasks、onedrive_account），只是沒有一個彙總畫面，不用逐頁點才知道有什麼要處理。"
    },
    {
  ...
  function taskItemHtml(lane, task, isDoneItem) {
    ...
    // 2026-07-15：使用者回報「我輸入完的內容應該要可以編輯」——原本
    // 送出之後文字就固定死了，打錯字/想補充只能刪掉重打（Claude 交辦
    // 的項目連刪都不能刪）。不分 origin，任何未完成項目都能編輯文字，
    // 跟「送出後不能改」這個舊限制沒有安全考量，純粹是漏做。
    var editButtonHtml = '<button type="button" class="jonaminz-journal-task-edit" data-task-edit aria-label="編輯">✎</button>';
    var editFormHtml =
      '<form class="jonaminz-journal-edit" data-edit-form hidden>' +
        '<input type="text" value="' + escapeHtml(task.text) + '" data-edit-input>' +
        '<button type="submit" aria-label="儲存">儲存</button>' +
      '</form>';
  ...
  function bindLaneEvents(lane) {
    ...
    if (clearBtn) {
      clearBtn.addEventListener("click", function () {
        // 只清 origin==='user' 的已完成項目——Claude 交辦的完成紀錄是
        // 永久保留的，Worker 端 clearDoneProjectTasks 也只會刪這種，
        // 這裡樂觀 UI 要跟後端行為一致，不然清完會出現「畫面上消失了
        // 但重新整理又跑回來」的錯覺。
        var removed = taskCache[lane].filter(function (t) { return t.done && t.origin === "user"; });
        if (!removed.length) {
          // 2026-07-15：使用者回報「已完成清除全部沒有反應」——原本這裡
          // 直接 return，如果已完成清單裡全部都是 Claude 交辦（永久保留、
          // 這支本來就不會清）的項目，按下去就真的什麼事都沒發生，跟
          // 「壞掉」沒兩樣。補一句明確提示，讓使用者知道這是設計如此，
          // 不是沒反應。
          var allDone = taskCache[lane].filter(function (t) { return t.done; });
          if (allDone.length) {
            window.alert("沒有可清除的項目——已完成清單裡目前都是 Claude 交辦的完成紀錄，會永久保留，不會被清除。");
          }
          return;
        }
        ...
        window.JonaminzBackend.clearDoneProjectTasks({ token: currentToken, lane: lane })
          .catch(function (error) {
            taskCache[lane] = taskCache[lane].concat(removed);
            renderLane(lane);
    ...
    board.addEventListener("click", function (event) {
      ...
      if (event.target.matches("[data-task-edit]")) {
        ...
        if (!editLi) return;
        var editForm = editLi.querySelector("[data-edit-form]");
        if (!editForm) return;
        editForm.hidden = !editForm.hidden;
        if (!editForm.hidden) {
          var editInput = editForm.querySelector("[data-edit-input]");
          if (editInput) {
            editInput.focus();
            editInput.select();
          }
    ...
    board.addEventListener("submit", function (event) {
      if (event.target.matches("[data-edit-form]")) {
        event.preventDefault();
        var editSubmitLi = event.target.closest("[data-task-id]");
        if (!editSubmitLi) return;
        var editSubmitFound = findTask(editSubmitLi.dataset.taskId);
        if (!editSubmitFound) return;
        var editSubmitInput = event.target.querySelector("[data-edit-input]");
        var newEditedText = editSubmitInput.value.trim();
        if (!newEditedText) return;
        var editedTask = taskCache[editSubmitFound.lane][editSubmitFound.index];
        var previousEditedText = editedTask.text;
        editedTask.text = newEditedText;
        renderLane(editSubmitFound.lane);

        window.JonaminzBackend.moveProjectTaskLane({ token: currentToken, id: editedTask.id, lane: editSubmitFound.lane, text: newEditedText })
          .catch(function (error) {
            editedTask.text = previousEditedText;
            renderLane(editSubmitFound.lane);
