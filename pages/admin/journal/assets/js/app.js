...
(function () {
  ...
  // 舊的）。
  var DECISION_TIMELINE_UPDATED_AT = "2026-07-17T00:01:00+08:00";
  ...
  var DECISION_TIMELINE = [
    ...
    },
    {
      ...
      tag: "onedrive",
      tagLabel: "OneDrive",
      title: "APK 上傳認證新增專用密鑰保管箱，不再每次跟使用者要個人 session token",
      summary: "createApkUploadSession 除了登入 session，也接受一把存在 agent_secrets 表、跟個人登入分開的密鑰；使用者自己在後台輸入名稱／值管理，像 Cloudflare secret 保管箱。",
      status: "done",
      detail: [
        ...
        { heading: "考慮的選項", branch: [
          ...
          { label: "Worker 自動產生單一把鑰匙、只看一次不能讀回（第一版）", text: "跟個人登入分開、不會過期、只給 createApkUploadSession 用，最小權限，存 app_settings 讓使用者自助輪替——上線後使用者當面回饋「不是這個意思」", chosen: false },
          { label: "通用密鑰保管箱，使用者自己輸入名稱/值（第二版，取代第一版）", text: "像 Cloudflare secret api 儲存那種模式：使用者自己選值存進 agent_secrets 表，需要換就自己上去改，不限定只能給 APK 上傳用；管理 action 只回傳名稱清單/更新時間，不吐出 value", chosen: true }
        ] },
        { heading: "決定與理由", text: "第一版做完後使用者當面糾正：他要的不是「Worker 自動產生、看一次就沒了」，而是「我把我手上的 token 自己存進去，你要用的時候進去拿，要換的時候我自己上去改」——改成新增 agent_secrets 表（跟不敏感的 app_settings 分開），listAgentSecrets/setAgentSecret/deleteAgentSecret 三支只給後台管理清單本身（新增/刪除/看名稱與時間），不回傳 value；真正讀 value 只有 Worker 內部程式碼查表（createApkUploadSession 透過 requireSessionOrAgentToken() 比對 name='apk_upload_token'），或 agent 對話中直接查表兩條路。" },
        { heading: "現況", text: "已上線（Worker Version dd851238）。tools/upload-apk.mjs 完全不用改，同一個 token 參數位置放個人 session token 或保管箱裡的密鑰都能用。過程中也補修了一個真實 bug：建 app_settings/agent_secrets 兩張表時漏了這個專案每張表都該有的 `grant ... to service_role`，導致第一次上線時讀取直接 403。" }
      ]
    },
    {
      id: "agent-secrets-own-page",
      date: "2026-07-16",
      tag: "process",
      tagLabel: "流程",
      title: "Agent 密鑰保管箱搬出工具包、獨立成頁＋畫面重做＋搬進既有 Supabase/Google 憑證",
      summary: "使用者指出工具包是「給人用的連結」、保管箱是「人存給 agent 用的憑證」，性質不同不該混頁；順便把散落的憑證檔案搬進來、畫面重畫。",
      status: "done",
      detail: [
        { heading: "情境", text: "使用者實際用過保管箱後追問三件事：(1) 這東西適不適合放工具包？(2) 「修改」是不是就是「覆蓋」？(3) 這個機制到底有沒有防到什麼，還是跟口頭叫 Claude 做事沒兩樣？同時要求把散落在 repo 根目錄的 suprabase db pw.txt／google oAuth pw.json 兩個敏感檔案裡「還會用到的」憑證搬進保管箱、其餘的判斷刪除。" },
        { heading: "考慮的選項", branch: [
          { label: "維持放在工具包頁", text: "省事，但工具包原本的定位是「人點的快速連結」，密鑰保管箱是完全不同性質的功能，混在一起使用者會搞不清楚這頁到底是做什麼的", chosen: false },
          { label: "獨立成 pages/admin/secrets/ 新頁", text: "照 pages/README.md 五步流程新增頁面，工具包恢復純靜態；後台首頁加一張新卡片連過去", chosen: true }
        ] },
        { heading: "決定與理由", text: "頁面拆分：新增 pages/admin/secrets/，工具包（app.js／CSS／config.json）復原成 2026-07-15 的純靜態版本。畫面重做：每筆密鑰改成卡片＋🔑符號，「覆蓋」（中性色）跟「刪除」（crimson 危險色，跟決策板既有的刪除按鈕配色一致）兩個動作分開，覆蓋按鈕點了直接把名稱帶進表單、游標跳到值欄位（使用者原話「就不用重打名稱」）。憑證搬移：讀 suprabase db pw.txt（DB 密碼＋一組標註 migration-temp 的 Management API token）跟 google oAuth pw.json（OAuth client_id/secret），使用者一開始說「你覺得用到哪些就存」，Auto Mode 安全機制擋下（判定使用者沒有具體點名要曝光哪些憑證），改成明確列出 4 筆讓使用者確認範圍；使用者回「你判斷不會用到的刪掉」，最終判斷：DB 密碼／Google client_id／client_secret 屬於還在用的基礎設施，搬進保管箱；那組 migration-temp 的 Management API token 沒有現成使用管道且標註臨時，不搬、只刪檔案（連帶提醒使用者這組 token 在 Supabase 那邊可能還活著，需要的話要自己去撤銷）。安全模型問答：使用者問「這樣有防到什麼嗎」——誠實回答這機制不驗證「現在講話的是不是你」（跟口頭指示的信任起點相同），真正的保護是「這把鑰匙外流時只能做這一件事」（requireSessionOrAgentToken() 只有 createApkUploadSession 一個呼叫端，其他 action 一律要求真的登入），是損害控制不是身分驗證。" },
        { heading: "現況", text: "已上線，純前端＋文件變更，沒有動 Worker 邏輯不用重新部署。suprabase db pw.txt／google oAuth pw.json 兩個檔案已從硬碟刪除（原本就不在 git 追蹤範圍內）。" }
      ]
    },
    {
      id: "bubble-leave-detection-multi-signal",
      date: "2026-07-16",
      tag: "bugfix",
      tagLabel: "泡泡",
      title: "泡泡失焦自動收合：單一訊號不可靠，改成四訊號並行",
      summary: "真機（Samsung SM-F9660）反覆驗證證實 OnWindowFocusChangeListener 常常不觸發或延遲近2分鐘，最終改成四種偵測訊號並行、任一觸發即收合。",
      status: "done",
      detail: [
        { heading: "情境", text: "懸浮泡泡（BubbleOverlayService，WindowManager overlay，不是 Activity）展開面板後，使用者離開（按 Home／切 App）該自動收合、只留泡泡，跟 Messenger 對齊。第一版用 OnWindowFocusChangeListener 偵測失焦，裝上真機後使用者回報「每次都不收回」，展開真機 adb 無線偵錯＋logcat 除錯（見 CHANGELOG 完整過程）。" },
        { heading: "考慮的選項", branch: [
          { label: "只修 OnWindowFocusChangeListener 本身", text: "第一輪以為是動畫 withEndAction 被系統凍結導致沒真的收合，改成關鍵旗標同步生效——這個修法是對的，但 logcat 證實這個監聽器本身在這台裝置上就常常不觸發或延遲極久，不只是動畫問題", chosen: false },
          { label: "改用 UsageStatsManager 輪詢最上層 App", text: "更穩定，但使用者一針見血抓到邏輯漏洞：我們的浮層不是 Activity，從桌面打開面板時桌面在系統眼裡從頭到尾沒被換掉過，「桌面開、按Home」這個情境用這招邏輯上不可能偵測到", chosen: false },
          { label: "四訊號並行，任一觸發即收合", text: "OnWindowFocusChangeListener（快速路徑）＋UsageStatsManager輪詢最上層App＋直接輪詢panelContainer.hasWindowFocus()（繞過回呼延遲）＋ACTION_CLOSE_SYSTEM_DIALOGS系統廣播（唯一不比對「誰是最上層」、直接對應「使用者按了Home/開最近工作列」這個系統動作本身的訊號，補上前三種都有死角的「桌面開、桌面按Home」情境）", chosen: true }
        ] },
        { heading: "決定與理由", text: "Android 刻意不讓 App 直接偵測 Home 鍵（防止惡意 App 困住使用者），只能用間接訊號推測「使用者離開了」——單一訊號在真機上都各自有死角（監聽器不穩定、輪詢比對邏輯在「本來就在桌面」時失效），沒有一種訊號能涵蓋所有情境，所以改成多訊號並行、任一觸發都收合。另外一起加了前景返回鍵收合（dispatchKeyEvent 攔截 KEYCODE_BACK，跟 Messenger 對齊，不穿透到底下的 App）。收合動畫維持原本的縮放/透明度效果，但關鍵的「不可觸控/不可對焦」視窗旗標改成立即同步生效、不等動畫 withEndAction——這是整個除錯過程最關鍵的一步，解決了三星 OneUI 背景凍結機制常讓動畫連第一格都播不完的根本問題。" },
        { heading: "現況", text: "已上線並經使用者真機反覆驗證確認（versionCode 15／202607161713，使用者原話「不錯很棒終於好了」）。新增 PACKAGE_USAGE_STATS 特殊權限（需使用者手動去系統設定開，App 內會自動導過去）。" }
      ]
    },
    {
      id: "task-board-scheduling",
      date: "2026-07-17",
      tag: "process",
      tagLabel: "流程",
      title: "待辦看板排程系統：純看板高亮、不推播、一次性到期",
      summary: "使用者想要「三個月後回來看那個 OneDrive」這種提醒。刻意做成最輕量——只在看板上倒數/逾期高亮，不推播、不自動搬動或刪除。",
      status: "done",
      detail: [
        { heading: "情境", text: "使用者在消化待辦時提出：希望待辦板能設定「到期時間」，用來記「隔一段時間要回來看某件事」（原話舉例三個月後回來看 OneDrive 那件事）。問題是「到期」可以做得很重（推播提醒、到期自動搬泳道、重複排程…），要先決定做多深。" },
        { heading: "考慮的選項", branch: [
          { label: "到期推播提醒", text: "到期時發 Web Push／系統通知主動提醒——功能強但要接排程觸發器（cron）＋推播基礎設施，複雜度高，且會變成另一個要維護的通知來源", chosen: false },
          { label: "到期自動處理", text: "到期自動把項目搬回某泳道或高亮置頂甚至刪除——會動到既有資料流，且「自動搬動」對兩人共用的看板容易造成困惑", chosen: false },
          { label: "純看板高亮、一次性", text: "due_at 只是給人看的時間標記，做一個獨立「排程區」集中顯示倒數、逾期紅框/24h 內琥珀框，其餘什麼都不做——不推播、不自動搬動/刪除，一次性（過了就是逾期高亮，使用者自己處理）", chosen: true }
        ] },
        { heading: "決定與理由", text: "用 AskUserQuestion 當面跟使用者確認，三個設計點都選了最輕量的一版：board-highlight-only（不推播）、一次性到期（不重複）、獨立排程區（不改既有泳道結構）。理由是這個需求的本質是「別忘記」而不是「準時被打斷」，純視覺高亮就夠用，不值得為它引進 cron／推播那層基礎設施，也避免自動搬動造成的共用看板混亂。實作：project_tasks 加 due_at 欄位、任務列 🕑 設定、獨立排程區倒數、30 秒重繪。" },
        { heading: "現況", text: "已上線（v0.46.43，Worker Version c43dcfa2）。migration＋action wiring＋前端邏輯各自驗過（Playwright harness 實測排序/倒數/高亮/設定/清除），認證路徑等使用者實測。" }
      ]
    }
  ];
  ...
  // ---------- 排程系統（2026-07-16）----------
  // 設計定案：純看板高亮、不推播、一次性到期。due_at 只是給人看的時間
  // 標記，到期不自動搬動/刪除任何東西。這幾支都是純顯示/格式化用的。
  ...
  // 把正的毫秒差值講成人話（只取最大的單位，天/小時/分鐘）。
  function humanizeDuration(ms) {
    var mins = Math.floor(ms / 60000);
    if (mins < 1) return "不到 1 分鐘";
    var days = Math.floor(mins / 1440);
    if (days >= 1) {
      var remHours = Math.floor((mins - days * 1440) / 60);
      return remHours >= 1 ? (days + " 天 " + remHours + " 小時") : (days + " 天");
    }
    var hours = Math.floor(mins / 60);
    if (hours >= 1) {
      var remMins = mins - hours * 60;
      return remMins >= 1 ? (hours + " 小時 " + remMins + " 分鐘") : (hours + " 小時");
    }
    return mins + " 分鐘";
  }

  // due_at → { label, state }。state：overdue（已過）/soon（24h 內）/later。
  function formatCountdown(iso) {
    var d = new Date(iso);
    if (isNaN(d.getTime())) return { label: "", state: "later" };
    var diff = d.getTime() - Date.now();
    if (diff <= 0) return { label: "已逾期 " + humanizeDuration(-diff), state: "overdue" };
    if (diff < 86400000) return { label: "還有 " + humanizeDuration(diff), state: "soon" };
    return { label: "還有 " + humanizeDuration(diff), state: "later" };
  }

  // due_at → 絕對時間人類可讀（給排程區顯示到期時刻）。
  function formatDueMoment(iso) {
    var d = new Date(iso);
    if (isNaN(d.getTime())) return "";
  ...
  function taskItemHtml(lane, task, isDoneItem) {
    ...
    // 排程：未完成項目才排程（已完成的不需要）。🕑 按鈕開一個 inline
    // datetime 表單，設定/清除 due_at。有 due_at 時列上顯示倒數 chip、
    // 依 overdue/soon 給 <li> 高亮 class。表單預設收合，跟 escalate 一樣
    // 用 [hidden]＋CSS 明確擋掉 UA display 的坑。
    var hasDue = !!task.due_at;
    var countdown = hasDue ? formatCountdown(task.due_at) : null;
    var dueClass = hasDue ? (" jonaminz-journal-task--due-" + countdown.state) : "";
    var scheduleFormHtml =
      '<form class="jonaminz-journal-schedule-form" data-schedule-form hidden>' +
        '<input type="datetime-local" data-schedule-input value="' + escapeHtml(toDatetimeLocalValue(task.due_at)) + '">' +
        '<button type="submit" aria-label="設定排程">設定</button>' +
        (hasDue ? '<button type="button" class="jonaminz-journal-schedule-clear" data-schedule-clear aria-label="清除排程">清除</button>' : "") +
      '</form>';
    var dueChipHtml = hasDue
      ? '<div class="jonaminz-journal-task-due" data-task-due>🕑 ' + escapeHtml(countdown.label) +
        '<span class="jonaminz-journal-task-due-moment">（' + escapeHtml(formatDueMoment(task.due_at)) + '）</span></div>'
      : "";
    var scheduleButtonHtml =
      '<button type="button" class="jonaminz-journal-task-schedule' + (hasDue ? " is-set" : "") +
      '" data-task-schedule aria-label="排程">🕑</button>';
    ...
    return (
      '<li class="jonaminz-journal-task' + (task._pending ? " is-pending" : "") + dueClass + '" data-task-id="' + escapeHtml(task.id) + '" data-task-text="' + escapeHtml(task.text) + '">' +
        bodyHtml +
        scheduleFormHtml +
        escalateFormHtml +
      '</li>'
  ...
  var LANE_LABEL = { for_user: "你要做的", for_claude: "你想叫我做的" };

  // 排程區：把兩條泳道裡所有「未完成且有 due_at」的項目集中顯示，依到期
  // 時間升冪排。這是一個獨立的檢視（項目本身還是活在原本的泳道），給
  // 「三個月後回來看那個 OneDrive」這種提醒用的——純看板高亮、不推播。
  function renderSchedule() {
    var listRoot = document.querySelector("[data-schedule-list]");
    var countRoot = document.querySelector("[data-schedule-count]");
    if (!listRoot) return;
    var scheduled = [];
    ["for_user", "for_claude"].forEach(function (lane) {
      taskCache[lane].forEach(function (task) {
        if (!task.done && task.due_at) scheduled.push(task);
      });
    });
    scheduled.sort(function (a, b) {
      return new Date(a.due_at).getTime() - new Date(b.due_at).getTime();
    });
    if (countRoot) countRoot.textContent = String(scheduled.length);
    if (!scheduled.length) {
      listRoot.innerHTML = '<li class="jonaminz-journal-empty">目前沒有排程項目。在任一待辦項目按 🕑 就能設定到期時間。</li>';
      return;
    }
    listRoot.innerHTML = scheduled.map(function (task) {
      var cd = formatCountdown(task.due_at);
      return (
        '<li class="jonaminz-journal-schedule-item jonaminz-journal-task--due-' + cd.state + '" data-task-id="' + escapeHtml(task.id) + '">' +
          '<div class="jonaminz-journal-schedule-main">' +
            '<span class="jonaminz-journal-schedule-lane">' + escapeHtml(LANE_LABEL[task.lane] || task.lane) + '</span>' +
            '<span class="jonaminz-journal-schedule-text">' + escapeHtml(task.text) + '</span>' +
          '</div>' +
          '<div class="jonaminz-journal-schedule-meta">' +
            '<span class="jonaminz-journal-schedule-countdown">' + escapeHtml(cd.label) + '</span>' +
            '<span class="jonaminz-journal-schedule-moment">' + escapeHtml(formatDueMoment(task.due_at)) + '</span>' +
            '<button type="button" class="jonaminz-journal-schedule-clear" data-schedule-jump-clear aria-label="清除排程">清除</button>' +
          '</div>' +
        '</li>'
      );
    }).join("");
  ...
  // 設定/清除一筆任務的 due_at（dueAtIso 傳 null 就是清除）。樂觀更新：
  // 先改 taskCache＋重繪，失敗才回滾。lane row 跟排程區共用這支。
  function commitDueAt(task, dueAtIso) {
    var previousDue = task.due_at || null;
    task.due_at = dueAtIso;
    renderBothLanes();
    window.JonaminzBackend.setProjectTaskDueAt({ token: currentToken, id: task.id, dueAt: dueAtIso })
      .then(function (response) {
        // Worker 會回傳正規化後的 dueAt（ISO 或 null），以它為準對齊。
        if (response && response.ok && Object.prototype.hasOwnProperty.call(response, "dueAt")) {
          task.due_at = response.dueAt;
          renderBothLanes();
        }
      })
      .catch(function (error) {
        task.due_at = previousDue;
        renderBothLanes();
        window.alert((dueAtIso ? "設定排程" : "清除排程") + "失敗：" + (error && error.message ? error.message : String(error)));
      });
  }
  ...
  function bindLaneEvents(lane) {
    ...
    board.addEventListener("click", function (event) {
      ...
      if (event.target.matches("[data-task-schedule]")) {
        var scheduleLi = event.target.closest("[data-task-id]");
        if (!scheduleLi) return;
        var scheduleForm = scheduleLi.querySelector("[data-schedule-form]");
        if (!scheduleForm) return;
        scheduleForm.hidden = !scheduleForm.hidden;
        event.target.classList.toggle("is-open", !scheduleForm.hidden);
        if (!scheduleForm.hidden) {
          var scheduleInput = scheduleForm.querySelector("[data-schedule-input]");
          if (scheduleInput) scheduleInput.focus();
        }
        return;
      }
      if (event.target.matches("[data-schedule-clear]")) {
        var clearLi = event.target.closest("[data-task-id]");
        if (!clearLi) return;
        var clearFound = findTask(clearLi.dataset.taskId);
        if (!clearFound) return;
        commitDueAt(taskCache[clearFound.lane][clearFound.index], null);
        return;
      }
    ...
    board.addEventListener("submit", function (event) {
      if (event.target.matches("[data-schedule-form]")) {
        event.preventDefault();
        var scheduleLi2 = event.target.closest("[data-task-id]");
        if (!scheduleLi2) return;
        var scheduleFound = findTask(scheduleLi2.dataset.taskId);
        if (!scheduleFound) return;
        var scheduleInput2 = event.target.querySelector("[data-schedule-input]");
        var rawValue = scheduleInput2 ? scheduleInput2.value : "";
        if (!rawValue) {
          window.alert("請先選一個到期時間。");
          return;
        }
        // datetime-local 的值沒有時區，new Date() 會當成本地時間解析，
        // toISOString() 轉成 UTC 存 DB——顯示時再轉回本地，兩邊一致。
        var parsed = new Date(rawValue);
        if (isNaN(parsed.getTime())) {
          window.alert("時間格式無法解析。");
          return;
        }
        commitDueAt(taskCache[scheduleFound.lane][scheduleFound.index], parsed.toISOString());
        return;
      }
  ...
  // 排程區的「清除」按鈕在待辦看板容器外（獨立 section），board 的事件
  // 代理抓不到，所以另外在排程清單上掛一個代理。倒數 interval 只重繪
  // 這個 section（不動 lane，避免打斷 lane 內開著的排程/編輯表單）。
  var scheduleBound = false;
  var scheduleTimer = null;
  function bindScheduleEvents() {
    var list = document.querySelector("[data-schedule-list]");
    if (!list || scheduleBound) return;
    scheduleBound = true;
    list.addEventListener("click", function (event) {
      if (!event.target.matches("[data-schedule-jump-clear]")) return;
      var li = event.target.closest("[data-task-id]");
      if (!li) return;
      var found = findTask(li.dataset.taskId);
      if (!found) return;
      commitDueAt(taskCache[found.lane][found.index], null);
    });
    if (scheduleTimer) clearInterval(scheduleTimer);
    // 30 秒重算一次倒數。只重繪排程 section，lane 裡的 chip 會在任何
    // lane 重繪（新增/勾選/設定排程等）時一起更新，不用每 30 秒動 lane。
    scheduleTimer = setInterval(renderSchedule, 30000);
  }
  ...
  function render() {
    ...
    root.innerHTML =
      '<section class="jonaminz-journal-board" data-journal-board>' +
        ...
        '</div>' +
      '</section>' +
      '<section class="jonaminz-journal-schedule-section">' +
        '<p class="jonaminz-admin-section-title">排程區' +
        '<span class="jonaminz-journal-schedule-badge" data-schedule-count>0</span></p>' +
        '<p class="jonaminz-page-subtitle">有設定到期時間的待辦會集中在這裡並倒數，逾期會高亮——純提醒用，不會推播，也不會自動搬動或刪除。</p>' +
        '<ul class="jonaminz-journal-schedule-list" data-schedule-list></ul>' +
      '</section>' +
