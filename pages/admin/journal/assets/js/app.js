...
(function () {
  ...
  // 按「加進你想叫我做的」會直接呼叫 addProjectTask（origin 預設
  // 'user'，因為是使用者自己選的，不是 Claude 片面塞進去的）。
  var DECISION_MAP = [
    {
      id: "toolkit-page",
      title: "後台「工具包」頁面",
    ...
    },
    {
      ...
      why: "上傳/下載/分享的 Graph 邏輯都已經現成（見 requestImageUpload/sendImageMessage/getImageUrls），主要工作是前端多一種「檔名+下載連結」卡片，不是圖片預覽那種。"
    },
    {
  ...
  function renderDecisionMap() {
    ...
    if (!root) return;
    root.innerHTML = DECISION_MAP.map(decisionMapCardHtml).join("");
  }
  ...
  function bindDecisionMapEvents() {
    ...
    root.addEventListener("click", function (event) {
      ...
      taskCache.for_claude.push({ id: tempId, lane: "for_claude", text: text, done: false, _pending: true, origin: "user" });
      renderLane("for_claude");
      window.JonaminzBackend.addProjectTask({ token: currentToken, lane: "for_claude", text: text })
  ...
  function render() {
    ...
    root.innerHTML =
      ...
      '<section class="jonaminz-journal-map-section">' +
        '<p class="jonaminz-admin-section-title">決策圖：接下來可以做的</p>' +
