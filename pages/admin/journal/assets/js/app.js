  ...
  function taskItemHtml(lane, task, isDoneItem) {
    ...
    var escalateFormHtml = lane === "for_user"
      ? '<form class="jonaminz-journal-escalate" data-escalate-form hidden>' +
          '<input type="text" placeholder="說明需要怎麼調整，按 Enter 送出..." data-escalate-input>' +
        '</form>'
      : "";
    return (
  ...
  function bindLaneEvents(lane) {
    ...
    if (clearBtn) {
      clearBtn.addEventListener("click", function () {
        var removed = taskCache[lane].filter(function (t) { return t.done; });
        if (!removed.length) return;
        taskCache[lane] = taskCache[lane].filter(function (t) { return !t.done; });
        renderLane(lane);
    ...
    // 「›」表單按 Enter 送出（單一輸入框的表單沒有送出按鈕，Enter 會
    // 觸發 submit，不需要另外的「送出」／「取消」按鈕——再按一次「›」
    // 就等於取消，不用多一個按鈕做同一件事）。把原文字＋使用者補充的
    // 調整說明，變成一筆新的 for_claude 項目，原本 for_user 那筆隨之
    // 移除（移過去，不是複製）。
    board.addEventListener("submit", function (event) {
      ...
      var original = taskCache[found.lane].splice(found.index, 1)[0];
      var newText = note ? (original.text + "：" + note) : original.text;

      var tempId = "temp-" + Date.now() + "-" + Math.random().toString(36).slice(2);
      taskCache.for_claude.push({ id: tempId, lane: "for_claude", text: newText, done: false, _pending: true });
      renderBothLanes();

      window.JonaminzBackend.addProjectTask({ token: currentToken, lane: "for_claude", text: newText })
        .then(function (response) {
          var tempFound = findTask(tempId);
          if (tempFound && response && response.row) {
            taskCache[tempFound.lane][tempFound.index] = response.row;
            renderLane(tempFound.lane);
          }
          return window.JonaminzBackend.deleteProjectTask({ token: currentToken, id: original.id });
        })
        .catch(function (error) {
          var tempFound2 = findTask(tempId);
          if (tempFound2) taskCache[tempFound2.lane].splice(tempFound2.index, 1);
          taskCache.for_user.push(original);
          renderBothLanes();
