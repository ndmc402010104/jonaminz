...
(function () {
  ...
  // 2026-07-16（使用者回饋「不要看到進入畫面還在跳畫面」）：這幾個
  // render*Attention/renderPendingStatus 函式原本是 fire-and-forget
  // （不 return promise），READY_TASK 提早 done、curtain 先掀開，
  // 角標/文字之後才彈出來造成視覺跳動。改成都 return 自己的 promise
  // chain，讓 init() 可以在放行 gate 前先等它們（跟 theme 用
  // Promise.race 設時間上限賽跑同一招，見下方 ATTENTION_CAP_MS 說明）。
  function renderPendingStatus() {
    ...
    return window.JonaminzBackend.listPendingContracts()
  ...
  function renderChatAttention() {
    ...
    if (!card || !window.JonaminzBackend || typeof window.JonaminzBackend.listChatMessages !== "function") return Promise.resolve();
  ...
  function renderVerifyAttention() {
    ...
    if (!card || !window.JonaminzBackend || typeof window.JonaminzBackend.listProjectTasks !== "function") return Promise.resolve();
    var token = (window.JonaminzIdentity && window.JonaminzIdentity.readToken)
      ? window.JonaminzIdentity.readToken() : null;
    return window.JonaminzBackend.listProjectTasks({ token: token })
      .then(function (response) {
        var rows = (response && response.rows) || [];
  ...
  function renderConnectionsAttention() {
    ...
    if (!card || !window.JonaminzBackend || typeof window.JonaminzBackend.getOnedriveStatus !== "function") return Promise.resolve();
    var token = (window.JonaminzIdentity && window.JonaminzIdentity.readToken)
      ? window.JonaminzIdentity.readToken() : null;
    return window.JonaminzBackend.getOnedriveStatus({ token: token })
      .then(function (response) {
  ...
  // 跟 entry-core.js 的 theme fetch 同一招（loadThemeWithCap()）：等這批
  // 角標判斷，但設時間上限賽跑，逾時就不再等——維持「背景資訊不該讓
  // 慢速/掛掉的後端無限期卡住整頁」的原意，只是從「完全不等」改成
  // 「等一下下」。
  var ATTENTION_CAP_MS = 1500;
  ...
  function init() {
    window.JonaminzIdentity.requireLogin().then(function (identity) {
      try {
        render(identity);
        renderRegistrations();
        var attentionSettled = Promise.all([
          renderPendingStatus(),
          renderChatAttention(),
          renderVerifyAttention(),
          renderConnectionsAttention()
        ]);
        var attentionCap = new Promise(function (resolve) { setTimeout(resolve, ATTENTION_CAP_MS); });
        Promise.race([attentionSettled, attentionCap]).then(function () {
