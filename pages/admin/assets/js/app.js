...
2026-07-16：新增「有什麼新的」摘要列（使用者從決策圖候選直接指定要做）
——未讀訊息數、待你驗證的任務數、OneDrive 連接狀態，三個都是點了就
直接跳去對應頁面的連結，不是純展示。三個資料來源都是既有 action
（listChatMessages／listProjectTasks／getOnedriveStatus）前端聚合，
不動 worker.js；未讀數算法直接照抄 pages/chat-launcher/index.html
既有的邏輯（比對 lastReadMessageId 之後、對方傳的訊息），保持跟浮動
大頭貼角標同一套定義，不要兩邊各自一套算法。跟 pending 數量／registrations
同一個原則：讀取失敗只在各自區塊顯示文字，不擋頁面本身的 all-ready。
*/
(function () {
  "use strict";
  ...
  function render(identity) {
    ...
    root.innerHTML =
      ...
      '</section>' +
      '<section class="jonaminz-admin-summary">' +
        '<p class="jonaminz-admin-section-title">有什麼新的</p>' +
        '<div class="jonaminz-admin-summary-grid">' +
          '<a class="jonaminz-admin-summary-item" href="/pages/chat/" data-summary-unread>讀取中...</a>' +
          '<a class="jonaminz-admin-summary-item" href="/pages/admin/journal/" data-summary-pending>讀取中...</a>' +
          '<a class="jonaminz-admin-summary-item" href="/pages/admin/connections/" data-summary-onedrive>讀取中...</a>' +
        '</div>' +
      '</section>' +
  ...
  // 2026-07-16：跟 pages/chat-launcher/index.html 的 refresh() 同一套
  // 未讀數算法（比對 identity 自己 lastReadMessageId 之後、對方傳的
  // 訊息）——刻意重複一份，理由跟這個專案裡其他 shell script 各自維護
  // 一份小工具一樣：這幾個入口彼此獨立，不互相依賴。
  function renderUnreadSummary() {
    var el = document.querySelector("[data-summary-unread]");
    if (!el) return;
    if (!window.JonaminzBackend || typeof window.JonaminzBackend.listChatMessages !== "function") {
      el.textContent = "Chat：讀取失敗";
      return;
    }
    ...
    window.JonaminzBackend.listChatMessages({ token: token })
      .then(function (data) {
        ...
        el.textContent = unreadCount > 0 ? ("Chat：" + unreadCount + " 則未讀") : "Chat：沒有未讀訊息";
      })
      .catch(function (error) {
        el.textContent = "Chat：讀取失敗（" + (error && error.message ? error.message : String(error)) + "）";
      });
  }

  // 「待你驗證」＝for_user 泳道裡 origin==='claude' 且還沒打勾的項目
  // ——跟決策與待辦頁的「請驗證：」慣例是同一批東西，不是泛指所有未完成
  // 待辦（使用者自己打的待辦不算「待驗證」，那些只是待處理）。
  function renderPendingTaskSummary() {
    var el = document.querySelector("[data-summary-pending]");
    if (!el) return;
    if (!window.JonaminzBackend || typeof window.JonaminzBackend.listProjectTasks !== "function") {
      el.textContent = "待驗證：讀取失敗";
      return;
    }
    var token = (window.JonaminzIdentity && window.JonaminzIdentity.readToken)
      ? window.JonaminzIdentity.readToken() : null;
    window.JonaminzBackend.listProjectTasks({ token: token })
      .then(function (response) {
        ...
        el.textContent = pendingCount > 0 ? (pendingCount + " 筆待你驗證") : "沒有待驗證項目";
      })
      .catch(function (error) {
        el.textContent = "待驗證：讀取失敗（" + (error && error.message ? error.message : String(error)) + "）";
      });
  }

  function renderOnedriveSummary() {
    var el = document.querySelector("[data-summary-onedrive]");
    if (!el) return;
    if (!window.JonaminzBackend || typeof window.JonaminzBackend.getOnedriveStatus !== "function") {
      el.textContent = "OneDrive：讀取失敗";
      return;
    }
    var token = (window.JonaminzIdentity && window.JonaminzIdentity.readToken)
      ? window.JonaminzIdentity.readToken() : null;
    window.JonaminzBackend.getOnedriveStatus({ token: token })
      .then(function (response) {
        ...
        if (connectedLabels.length === 2) {
          el.textContent = "OneDrive：兩人都已連接";
        } else if (connectedLabels.length === 1) {
          el.textContent = "OneDrive：只有 " + connectedLabels[0] + " 已連接";
        } else {
          el.textContent = "OneDrive：都還沒連接";
        }
      })
      .catch(function (error) {
        el.textContent = "OneDrive：讀取失敗（" + (error && error.message ? error.message : String(error)) + "）";
      });
  }
  ...
  function init() {
    window.JonaminzIdentity.requireLogin().then(function (identity) {
      try {
        ...
        renderRegistrations();
        renderUnreadSummary();
        renderPendingTaskSummary();
        renderOnedriveSummary();
      } catch (error) {
